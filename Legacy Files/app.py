...
# Load OpenAI API key from file "openai_api_key.json"
...
def update_npc_memory():
    ...
    # Append the new interaction to the rag_log
    memory["rag_log"].append({
        "interaction": interaction,
        "timestamp": timestamp
    })

    # Compute cutoff time as offset-aware datetime in UTC
...
def create_snapshot():
    ...
    # Retrieve the current state from the specified source path
    ref = db.reference(source_path)
    state = ref.get()
    if state is None:
        return jsonify({"error": "No state found at the specified source_path"}), 404

    # Create a timestamp in a safe format (no colons or dots)
    # Example format: "2025-04-04T13-55-07-503497"
...
def npc_interact():
    """
    Process an NPC interaction using the GPT API.
    ...
    # Construct a full prompt including context
    full_prompt = (
        f"NPC ID: {npc_id}\n"
    ...
    try:
        ...
        return jsonify({"npc_id": npc_id, "reply": reply})
    except Exception as e:
        return jsonify({"error": "Failed to generate NPC response", "details": str(e)}), 500

@app.route('/dm_response', methods=['POST'])
def dm_response():
    """
    Generate a DM narrative response using the GPT API.
    
    Expected JSON input:
    {
      "context": "Relevant world state, events, or previous dialogue",
    ...
    try:
        ...
        return jsonify({"error": "Failed to generate DM response", "details": str(e)}), 500
...
def move_player():
    ...
    # Reference to the player's state in Firebase
    ref = db.reference(f'/players/{player_id}')
    player_state = ref.get() or {}
    
    # Update the player's location
    player_state["location"] = new_location
    ref.set(player_state)
    
    return jsonify({"message": f"Player {player_id} moved to {new_location}", "player_state": player_state})
