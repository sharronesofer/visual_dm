...
class WorldGenerator:
    ...
    def __init__(self) -> None:
        ...
        self._biome_data = {
            'forest': BiomeConfig(
                enemy_types=['goblin', 'wolf', 'bandit'],
                resource_types=['wood', 'herbs', 'game'],
                terrain=['hills', 'trees', 'rivers'],
                features=['clearing', 'grove', 'stream'],
                resources={'wood': 0.8, 'herbs': 0.6, 'game': 0.4}
            ),
            'desert': BiomeConfig(
                enemy_types=['scorpion', 'bandit', 'undead'],
                resource_types=['minerals', 'cactus', 'relics'],
                terrain=['dunes', 'oasis', 'ruins'],
                features=['dune', 'oasis', 'ruin'],
                resources={'minerals': 0.7, 'cactus': 0.5, 'relics': 0.3}
            ),
            'mountain': BiomeConfig(
                enemy_types=['troll', 'eagle', 'dwarf'],
                resource_types=['ore', 'gems', 'crystals'],
                terrain=['peaks', 'caves', 'cliffs'],
                features=['peak', 'cave', 'cliff'],
                resources={'ore': 0.9, 'gems': 0.4, 'crystals': 0.2}
            )
        }
    ...
    def generate_region(
        ...
        """Generate a game region with caching."""
        try:
            # Size multipliers
            size_multipliers = {
                'small': 0.5,
            ...
            biome_info = self._biome_data.get(biome, self._biome_data['forest'])
            
            # Generate region features with parallel processing
            features = {
                'level': level,
                'biome': biome,
                'size': size,
                'enemies': self._generate_enemies(
                    biome_info.enemy_types,
                    level,
                    size_multipliers[size]
                ),
                'resources': self._generate_resources(
                    biome_info.resource_types,
                    level,
                    size_multipliers[size]
                ),
