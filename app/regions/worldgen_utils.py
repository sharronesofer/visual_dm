...
class WorldGenerator:
    ...
    def __init__(self, seed: Optional[int] = None):
        ...
        self.biome_configs = self._load_biome_configs()
        
    def _load_biome_configs(self) -> Dict[str, BiomeConfig]:
        """Load biome configurations."""
        return {
            'forest': BiomeConfig(
                name='forest',
                ...
                features=['trees', 'rivers', 'wildlife'],
                resources={'wood': 0.8, 'berries': 0.6}
            ),
            'desert': BiomeConfig(
                name='desert',
                ...
                features=['sand_dunes', 'cacti', 'oasis'],
                resources={'sand': 0.9, 'crystals': 0.3}
            ),
            'mountain': BiomeConfig(
                name='mountain',
                temperature_range=(0.0, 0.5),
                humidity_range=(0.3, 0.7),
                elevation_range=(0.7, 1.0),
                features=['peaks', 'caves', 'cliffs'],
                resources={'stone': 0.9, 'minerals': 0.7}
            )
        }
    ...
    def _determine_biome(self, elevation: float, temperature: float, humidity: float) -> str:
        ...
        try:
            ...
            scores = {}
            for biome_name, config in self.biome_configs.items():
                temp_score = self._calculate_factor_score(
                    temperature,
                    config.temperature_range
                )
                humid_score = self._calculate_factor_score(
                    humidity,
                    config.humidity_range
                )
                elev_score = self._calculate_factor_score(
                    elevation,
    ...
    def _generate_features(self, biome_config: BiomeConfig) -> List[str]:
        """Generate features for a biome."""
        try:
            features = []
            for feature in biome_config.features:
                # Higher chance for common features
    ...
    def _generate_resources(self, biome_config: BiomeConfig) -> Dict[str, float]:
        """Generate resources for a biome."""
        try:
            resources = {}
            for resource, base_chance in biome_config.resources.items():
