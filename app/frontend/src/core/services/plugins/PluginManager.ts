...
export class PluginManager extends EventEmitter {
  ...
  private validatePlugin(plugin: unknown): plugin is ProcessingPlugin {
    // Enforce coordinate standardization for plugins exposing spatial data
    if (plugin && typeof plugin === 'object' && 'position' in plugin) {
      const pos = (plugin as any).position;
      if (!pos || (typeof pos !== 'object') || (!('x' in pos && 'y' in pos))) {
        throw new ServiceError(
          'PluginValidationError',
          `Plugin ${(plugin as any).name} must expose position as a standardized coordinate type (GlobalCoord, LocalCoord, or wrapper)`,
          { plugin }
        );
      }
      // Optionally, check for class instance or type tag
      if (!('type' in pos) || (pos.type !== 'GlobalCoord' && pos.type !== 'LocalCoord')) {
        throw new ServiceError(
          'PluginValidationError',
          `Plugin ${(plugin as any).name} position must be of type GlobalCoord or LocalCoord`,
          { plugin }
        );
      }
    }
    return (
      ...
      Object.values(PluginType).includes((plugin as ProcessingPlugin).type as PluginType) &&
      'initialize' in plugin &&
      typeof (plugin as ProcessingPlugin).initialize === 'function' &&
      'cleanup' in plugin &&
      typeof (plugin as ProcessingPlugin).cleanup === 'function'
    );
  }
