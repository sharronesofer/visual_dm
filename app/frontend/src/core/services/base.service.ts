...
export interface BaseQueryParams {
  ...
  order?: 'asc' | 'desc';
  search?: string;
  filter?: Record<string, MetadataValue>;
...
export abstract class BaseService<T extends BaseEntity> {
  ...
  protected readonly retryableStatuses: number[];

  constructor(resourcePath: string, options: BaseServiceOptions = {}) {
    const {
      baseURL,
      cacheEnabled = true,
  ...
  async warmCache(data: T | T[], params?: BaseQueryParams): Promise<void> {
    ...
    if (Array.isArray(data)) {
      ...
      await this.cache.set(cacheKey, { data, meta: { total: data.length } }, this.cacheTTL);
    } else {
   ...
   */
  async get(id: string | number, cacheOptions: CacheOptions = {}): Promise<ServiceResponse<T>> {
    const {
      enabled = this.cacheEnabled,
      ttl = this.cacheTTL,
      invalidateOnError = true,
   ...
   */
  async delete(id: string | number): Promise<ServiceResponse<T>> {
    const response = await this.executeWithRetry(async () => {
