...
import { StorageError, StorageErrorCode } from '../../../interfaces/storage/StorageProvider';
...
describe('S3StorageProvider', () => {
  ...
  describe('save', () => {
    describe('regular upload', () => {
      it('should upload small files using PutObject', async () => {
        ...
        expect(s3Mock.calls()).toHaveLength(2); // PutObject + GetObjectAttributes
    ...
    describe('multipart upload', () => {
      ...
      it('should use multipart upload for large files', async () => {
        ...
        expect(uploadCalls.length).toBeGreaterThan(1); // Multiple parts
  ...
  describe('get', () => {
    it('should retrieve file data', async () => {
      ...
      s3Mock.on(GetObjectCommand).resolves({
        Body: mockStream as any // Type assertion needed as AWS SDK expects their own stream type
      });
  ...
  describe('getUrl', () => {
    ...
    it('should generate public URL for public files', async () => {
      const url = await provider.getUrl('public.txt', { expiresIn: undefined }); // Use undefined to get a public URL
      expect(url).not.toContain('X-Amz-Algorithm'); // Not a presigned URL
      expect(url).toContain('test-bucket');
