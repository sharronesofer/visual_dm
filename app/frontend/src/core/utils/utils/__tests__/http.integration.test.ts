...
import { httpClient, createHttpClient, createCancellationToken } from '../http';
...
describe('HTTP Utilities Integration Tests', () => {
  ...
  describe('Request Cancellation', () => {
    it('should cancel requests using cancellation token', async () => {
      ...
      mock.onGet('/test').reply(() => {
        return new Promise(() => {}); // Never resolve to test cancellation
      });
  ...
  describe('Timeout Handling', () => {
    it('should timeout requests after specified duration', async () => {
      ...
      mock.onGet('/test').reply(() => {
        return new Promise(() => {}); // Never resolve to test timeout
      });
  ...
  describe('Retry Logic', () => {
    ...
    it('should stop retrying after max attempts', async () => {
      ...
      expect(mock.history.get.length).toBe(4); // Initial + 3 retries
    });
