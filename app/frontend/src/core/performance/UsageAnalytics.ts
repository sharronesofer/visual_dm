...
export class UsageAnalytics {
  ...
  private isEnabled: boolean = API_METRICS.ENABLE_USAGE_TRACKING;
  ...
  private trackInteraction(action: string, element: HTMLElement): void {
  ...
  public trackEvent(type: string, action: string, metadata?: Record<string, string>, duration?: number): void {
    if (!this.isEnabled) return;
  ...
  private recordEventMetrics(event: UsageEvent): void {
    ...
    // Record event duration if available
    if (event.duration) {
      this.optimizer.recordMetric({
  ...
  public getFeatureUsage(featureId?: string): FeatureUsage | Map<string, FeatureUsage> {
    if (featureId) {
