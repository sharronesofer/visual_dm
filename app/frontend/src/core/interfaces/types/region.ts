...
export interface IRegion {
  id: string;
  name: string;
  boundaries: [number, number][][]; // Array of polygon rings as [lng, lat] coordinates
  style: {
    fillColor?: string;
    strokeColor?: string;
    strokeWidth?: number;
  ...
  getBoundingBox(): [[number, number], [number, number]]; // Returns [[minLng, minLat], [maxLng, maxLat]]
}
...
export class Region implements IRegion {
   ...
   */
  containsPoint(point: [number, number]): boolean {
    // Only checks the outer ring for simplicity
    const [lng, lat] = point;
