...
export class PathfindingSystem {
  ...
  public isPathPossible(start: GridPosition, end: GridPosition): boolean {
    return this.findPath(start, end).length > 0;
  }

  public findAccessibleArea(start: GridPosition, maxDistance: number): Set<string> {
    const accessible = new Set<string>();
    const queue: { position: GridPosition; distance: number }[] = [
  ...
  private calculateHeuristic(start: GridPosition, end: GridPosition): number {
