...
namespace VisualDM.Systems.FeatHistory
{
    ...
    public class FeatRecommendationEngine
    {
        ...
        // Systems capture (stub)
        public void RecordSystems(string characterId, string featId, bool accepted)
        {
            // In production, store feedback for future tuning
            // For now, just log
            Console.WriteLine($"[RecommendationEngine] Systems: {characterId} {(accepted ? "accepted" : "ignored")} {featId}");
        }
    }
} 