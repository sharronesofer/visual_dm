...
namespace VisualDM.Systems.Quest
{
    ...
    public class CrossRegionArcToQuestMapper : IArcToQuestMapper
    {
        private readonly List<ArcQuestMappingRule> mappingRules;

        public CrossRegionArcToQuestMapper(IEnumerable<ArcQuestMappingRule> rules = null)
        {
            mappingRules = rules != null ? new List<ArcQuestMappingRule>(rules) : new List<ArcQuestMappingRule> { new DefaultCrossRegionArcMappingRule() };
        }
        ...
        public List<Quest> GenerateQuests(ArcQuestGenerationContext context)
        {
            ...
            // Register cross-region quest state with GlobalArcManager
            foreach (var quest in quests)
            {
                GlobalArcManager.Instance.SyncQuestState(quest.Id, quest);
            }
            return quests;
        }
    }
    ...
    /// <summary>
    /// Example mapping rule for cross-region arcs.
    /// </summary>
    public class DefaultCrossRegionArcMappingRule : ArcQuestMappingRule
    {
        public DefaultCrossRegionArcMappingRule() { RuleName = "DefaultCrossRegionArcStageToQuest"; }

        public override bool IsApplicable(ArcQuestGenerationContext context)
        {
            return context.ArcType == "Global" && context.Arc is GlobalArc && context.ArcStage is ArcStage && context.WorldState.ContainsKey("CrossRegion");
        }
        ...
        public override void Apply(ArcQuestGenerationContext context, List<Quest> quests)
        {
            ...
            {
                ArcType = "Global",
                QuestType = "CrossRegion",
                BaseTitle = $"{arc.Title}: {stage.Name} {motifTag}",
                BaseDescription = stage.Description + (motifDesc != null ? $"\nMotif: {motifDesc}" : string.Empty),
                ObjectiveTemplates = new List<string> { $"Complete cross-region stage: {stage.Name} {motifTag}" }
            };
            var quest = template.GenerateQuest();
            // Mark quest as cross-region
            quest.Metadata["CrossRegion"] = true;
            quests.Add(quest);
        }
    }
}