...
namespace VisualDM.UI
{
    ...
    public class UIManager : MonoBehaviour
    {
        ...
        public void ShowEconomyDashboard(EconomySystem economySystem)
        {
            const string panelName = "EconomyDashboardPanel";
            if (!panels.ContainsKey(panelName))
            {
                var go = new GameObject(panelName);
                go.transform.SetParent(transform);
                var panel = go.AddComponent<EconomyDashboardPanel>();
                panel.Initialize(economySystem);
                panels[panelName] = go;
            }
            ShowPanel(panelName);
        }
