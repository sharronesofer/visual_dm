...
using System;
using UnityEngine;
using VDM.Runtime.Core.WebSocket;
...
namespace VDM.Runtime.Dialogue.Services
{
    ...
    public class DialogueWebSocketHandler : BaseWebSocketHandler
    {
        ...
        protected override void HandleMessage(string eventType, Dictionary<string, object> data)
        {
            try
            {
                switch (eventType)
            ...
            {
                Debug.LogError($"Error handling dialogue message {eventType}: {ex.Message}");
            }
        }
