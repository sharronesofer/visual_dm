...
namespace VDM.Runtime.Integration
{
    ...
    /// </summary>
    public class QuestArcIntegrationManager : MonoBehaviour
    {
        ...
        // Events
        public event Action<QuestModel, ArcModel> OnQuestArcLinked;
        public event Action<QuestModel, ArcModel> OnQuestArcUnlinked;
        public event Action<string, List<QuestModel>, List<ArcModel>> OnCharacterNarrativeUpdated;
        ...
        private void BuildMappingRelationships(List<QuestModel> quests, List<ArcModel> arcs)
        {
            questToArcsMapping.Clear();
            arcToQuestsMapping.Clear();
            characterArcMapping.Clear();
            
            // Build Quest -> Arc mappings
            foreach (var quest in quests)
            {
                ...
                {
                    questToArcsMapping[quest.Id].Add(quest.ArcId);
                    
                    // Build reverse mapping
                    if (!arcToQuestsMapping.ContainsKey(quest.ArcId))
                        arcToQuestsMapping[quest.ArcId] = new List<string>();
                    
                    arcToQuestsMapping[quest.ArcId].Add(quest.Id);
                }
            }
            
            // Build Character -> Arc mappings
            foreach (var arc in arcs)
            {
                ...
                // Ensure arc has quest mapping entry
        ...
        private async System.Threading.Tasks.Task ValidateAndRepairMappings(List<QuestModel> quests, List<ArcModel> arcs)
        {
        ...
        private void HandleQuestCreated(QuestModel quest)
        {
        ...
        private void HandleQuestCompleted(QuestModel quest)
        {
            if (isProcessingIntegrationEvent) return;
        ...
        private void HandleQuestFailed(QuestModel quest)
        {
            if (isProcessingIntegrationEvent) return;
        ...
        private void HandleQuestStepCompleted(QuestModel quest, QuestStepModel step)
        {
            if (isProcessingIntegrationEvent) return;
        ...
        private void HandleArcStepUpdated(string arcId, ArcStepModel step)
        {
            if (isProcessingIntegrationEvent) return;
        ...
        private void HandleArcProgressionUpdated(ArcProgressionModel progression)
        {
            if (isProcessingIntegrationEvent) return;
        ...
        private async System.Threading.Tasks.Task CheckArcProgression(QuestModel quest)
        {
            try
            {
                if (string.IsNullOrEmpty(quest.ArcId))
        ...
        private async System.Threading.Tasks.Task CheckArcImpact(QuestModel quest, string impactType)
        {
            try
            {
                if (string.IsNullOrEmpty(quest.ArcId))
        ...
        private async System.Threading.Tasks.Task CheckArcStepCorrelations(QuestModel quest, QuestStepModel questStep)
        {
            try
            {
                if (string.IsNullOrEmpty(quest.ArcId))
        ...
        private async System.Threading.Tasks.Task CheckQuestCorrelationsForArcStep(string arcId, ArcStepModel arcStep)
        {
            try
            {
        ...
        private async System.Threading.Tasks.Task UpdateQuestProgressionFromArc(ArcProgressionModel progression)
        {
            try
            {
        ...
        private async System.Threading.Tasks.Task<List<QuestModel>> GetQuestsForCharacter(string characterId)
        {
            try
            {
        ...
        private async System.Threading.Tasks.Task<List<QuestModel>> GetQuestsForArc(string arcId)
        {
            try
            {
                if (!arcToQuestsMapping.ContainsKey(arcId))
        ...
        /// </summary>
        public async System.Threading.Tasks.Task<List<QuestModel>> GetLinkedQuestsAsync(string arcId)
        {
