...
namespace VDM.Systems.Analytics.Services
{
    /// <summary>
    /// Analytics service for frontend-backend communication and data management
    /// </summary>
    public class AnalyticsService : BaseHttpService, ISystemService
    {
        ...
        private float frameTimeAccumulator = 0.0f;
        ...
        private void Update()
        {
            ...
            if (UnityEngine.Time.time - lastRefreshTime >= refreshInterval)
            {
            ...
            frameTimeAccumulator += UnityEngine.Time.deltaTime;
        }
        ...
        public async Task<AnalyticsReport> GenerateReport(DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                var response = await PostAsync($"{baseUrl}/reports/generate", requestData);
        ...
        public async Task<UserBehaviorData> GetUserBehaviorData(string userId = null)
        {
            try
            {
                ...
                var response = await GetAsync($"{baseUrl}/users/{userId}/behavior");
        ...
        private void UpdatePerformanceMetrics()
        {
            if (trackFrameRate)
            {
                currentMetrics.averageFrameRate = 1.0f / UnityEngine.Time.deltaTime;
            }
        ...
        private async Task UpdateSystemHealth()
        {
            try
            {
                var response = await GetAsync($"{baseUrl}/health");
                if (response.IsSuccess)
                {
        ...
        private async void SendEventToBackend(AnalyticsEvent analyticsEvent)
        {
            try
            {
                ...
                await PostAsync($"{baseUrl}/events", eventData);
            }
            catch (Exception ex)
            {
