...
namespace VDM.Systems.Religion.Ui
{
    ...
    /// </summary>
    public class ReligionPanelController : BaseUIComponent
    {
        ...
        public async void RefreshReligions()
        {
            try
            {
                _religions = await _religionService.GetReligionsAsync();
                ApplyFilters();
                UpdateReligionList();
            }
            catch (Exception ex)
        ...
        private async void LoadEntityMemberships(string entityId)
        {
            try
            {
                ...
                Debug.Log($"Loading memberships for entity: {entityId}");
            }
            catch (Exception ex)
            {
        ...
        private async void SaveReligion()
        {
            try
            {
                ...
                if (result != null)
                {
                    OnReligionCreated?.Invoke(result);
                    await RefreshReligions();
                    ShowListPanel();
                }
            }
        ...
        private async void DeleteSelectedReligion()
        {
            ...
            try
            {
                var result = await _religionService.DeleteReligionAsync(_selectedReligion.Id);
                if (result)
                {
        ...
        private async void JoinSelectedReligion()
        {
            ...
            try
            {
                ...
                // This would need to be implemented when entity membership system is available
                Debug.Log($"Join religion {_selectedReligion.Name} with devotion {devotion}");
            }
            catch (Exception ex)
            {
        ...
        private async void LeaveSelectedReligion()
        {
            ...
            {
                // This would need to be implemented when entity membership system is available
                Debug.Log($"Leave religion {_selectedReligion.Name}");
            }
            catch (Exception ex)
            {
        ...
        {
            // Initialize religion panel UI components
            if (_religionService == null)
            {
                _religionService = FindObjectOfType<ReligionService>();
                if (_religionService == null)
                {
                    Debug.LogError("ReligionPanelController: ReligionService not found");
                }
            }
        ...
        {
            // Set up UI button listeners here
            if (createReligionButton != null)
                createReligionButton.onClick.AddListener(OnCreateReligionClicked);
        ...
        {
            // Initialize any dropdown components
        }
        ...
        private async void LoadInitialData()
        {
            ...
            {
                Debug.LogError($"Failed to load initial religion data: {ex.Message}");
            }
        }
        ...
        private void OnCreateReligionClicked()
        {
            ...
            Debug.Log("Create religion button clicked");
        }
