...
namespace VDM.Systems.Combat.Services
{
    /// <summary>
    ...
    public class CombatWebSocketHandler : BaseWebSocketHandler
    {
        ...
        public event Action<string, List<VDM.DTOs.Combat.CombatActionDTO>> OnAvailableActionsUpdated;
        ...
        {
            Connect("ws://localhost:8000/ws/combat");
            Debug.Log("Combat WebSocket started");
        ...
        protected void OnConnectedInternal()
        {
            Debug.Log("Combat WebSocket connected");
        }
        
        protected void OnDisconnectedInternal()
        {
            ...
            try
            {
                ...
                if (data.payload.participant != null)
                {
                    combatEvent.Participant = new Combatant(
                        data.payload.participant.name ?? "Unknown", 
                        data.payload.participant.max_health ?? 100, 
                        data.payload.participant.armor_class ?? 10)
                    {
                        CurrentHP = data.payload.participant.current_health ?? 0,
                        MaxHP = data.payload.participant.max_health ?? 0,
                ...
                if (data.payload.participants != null)
                {
                    foreach (var participant in data.payload.participants)
                    {
                        var combatant = new Combatant(
                            ...
                            participant.max_health ?? 100,
                            participant.armor_class ?? 10)
                        {
                            CurrentHP = participant.current_health ?? 0,
                            MaxHP = participant.max_health ?? 0,
        ...
        private void HandleActionAvailable(dynamic data)
        {
            try
            {
                ...
                var actions = new List<VDM.DTOs.Combat.CombatActionDTO>();
