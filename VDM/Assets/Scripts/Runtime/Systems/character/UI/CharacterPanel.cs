...
namespace VDM.Systems.Character.Ui
{
    ...
    public class CharacterPanel : UIPanel
    {
        ...
        private void UpdateStatsDisplay()
        {
            ...
            // Health
            SetTextSafe(hpText, $"{stats.HitPoints}/{stats.MaxHitPoints}");
            SetSliderValue(hpSlider, stats.HitPoints, stats.MaxHitPoints);

            // Mana/Magic Points (not available in current DTO, so use placeholder values)
            SetTextSafe(mpText, "N/A");
        ...
        private void UpdateDetailsDisplay()
        {
            ...
            {
                SetTextSafe(descriptionText, _currentCharacter.Narrative.Backstory);
                SetTextSafe(goalsText, _currentCharacter.Narrative.CurrentGoal);
            }

            if (_currentCharacter.Personality != null)
            {
                // Combine personality traits into a description
                var personalityDesc = string.Join(", ", _currentCharacter.Personality.Traits);
                SetTextSafe(personalityText, personalityDesc);
            }

            // Metadata is not a dictionary, so we can't use ContainsKey
            // For now, just leave notes empty or use a placeholder
            SetTextSafe(notesText, "No additional notes available");
        }
