...
namespace VDM.Systems.Worldgeneration.Ui
{
    ...
    public class WorldGenerationPanel : MonoBehaviour
    {
        ...
        private List<BiomeConfigDTO> biomes = new List<BiomeConfigDTO>();
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogWarning("WorldGenerationPanel: WorldGenerationWebSocketHandler not found, some features may not work");
            }
        }
        ...
        private async void LoadInitialData()
        {
            ...
            {
                // Load templates
                var templatesResponse = await worldGenService.GetWorldTemplatesAsync();
        ...
        private async void OnAddContinentButtonClicked()
        {
            try
            {
                ...
                var result = await worldGenService.CreateContinentAsync(config);
                if (result != null)
                {
                    AddLogEntry($"Created continent: {result.Name}", LogLevel.Info);
                    await RefreshContinents();
                }
                else
                {
                    AddLogEntry($"Failed to create continent", LogLevel.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private async System.Threading.Tasks.Task StartWorldGeneration(WorldGenerationConfig config)
        {
            ...
            if (result != null)
            {
                AddLogEntry($"World generation completed: {result.Name}", LogLevel.Success);
                await RefreshContinents();
            }
            else
            {
                SetGenerationState(false);
                AddLogEntry("Failed to start generation", LogLevel.Error);
            }
        }
        ...
        private WorldGenerationConfig CreateGenerationConfig()
        {
            return new WorldGenerationConfig
            {
                ...
                Metadata = new Dictionary<string, object>
                {
                    ["description"] = worldDescriptionInput.text
                },
                BiomeParams = new BiomeGenerationParams
                {
                    BiomeDiversity = biomeDiversitySlider.value,
                    // Note: ResourceDensity is in SettlementParams, not BiomeParams
                },
                SettlementParams = new SettlementGenerationParams
                {
                    SettlementDensity = settlementDensitySlider.value,
                    ResourceDensity = resourceAbundanceSlider.value
                },
                // Note: templateId would need to be handled differently
                // as WorldGenerationConfig doesn't have a templateId property
            };
        }
        ...
        private void UpdateProgressDisplay()
        {
            ...
            statusText.text = currentProgress.Stage;
            progressText.text = $"{currentProgress.OverallProgress:P0} - {currentProgress.Stage}";

            if (!string.IsNullOrEmpty(currentProgress.CurrentTask))
            {
                AddLogEntry(currentProgress.CurrentTask, LogLevel.Info);
            }
        }
        ...
        private async System.Threading.Tasks.Task RefreshContinents()
        {
            try
            {
                continents = await worldGenService.GetContinentsAsync();
                RefreshContinentList();
            }
            catch (Exception ex)
            {
        ...
        private async void OnContinentDeleted(ContinentModel continent)
        {
            try
            {
                var success = await worldGenService.DeleteContinentAsync(continent.id);
                if (success)
                {
                ...
                {
                    AddLogEntry($"Failed to delete continent: {continent.name}", LogLevel.Error);
                }
            }
            catch (Exception ex)
            {
