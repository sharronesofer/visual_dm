...
namespace VDM.Runtime.Equipment.Services
{
    ...
    public class EquipmentWebSocketHandler : BaseWebSocketHandler
    {
        ...
        public event Action<string, Inventory> OnInventoryUpdated;
        public event Action<string, Equipment> OnItemEquipped;
        public event Action<string, EquipmentSlot> OnItemUnequipped;
        public event Action<Equipment> OnItemCreated;
        public event Action<string> OnItemDestroyed;
        public event Action<Equipment> OnItemEvolved;
        public event Action<Equipment> OnItemRepaired;
        public event Action<Equipment> OnItemIdentified;
        ...
        protected override void HandleMessage(string eventType, string data)
        {
            try
            {
                switch (eventType)
                {
                    case "equipment_event":
                        HandleEquipmentEventMessage(data);
                        break;
                    case "equipment_state_updated":
                        HandleEquipmentStateMessage(data);
                        break;
                    case "inventory_updated":
                        HandleInventoryUpdatedMessage(data);
                        break;
                    case "item_equipped":
                        HandleItemEquippedMessage(data);
                        break;
                    case "item_unequipped":
                        HandleItemUnequippedMessage(data);
                        break;
                    case "item_created":
                        HandleItemCreatedMessage(data);
                        break;
                    case "item_destroyed":
                        HandleItemDestroyedMessage(data);
                        break;
                    case "item_evolved":
                        HandleItemEvolvedMessage(data);
                        break;
                    case "item_repaired":
                        HandleItemRepairedMessage(data);
                        break;
                    case "item_identified":
                        HandleItemIdentifiedMessage(data);
                        break;
                    case "property_activated":
                        HandlePropertyActivatedMessage(data);
                        break;
                    case "property_deactivated":
                        HandlePropertyDeactivatedMessage(data);
                        break;
                    default:
                        base.HandleMessage(eventType, data);
                        break;
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Error handling equipment message '{eventType}': {ex.Message}");
            }
        }

        private void HandleEquipmentEventMessage(string data)
        {
            try
            {
                var equipmentEvent = JsonUtility.FromJson<EquipmentEvent>(data);
                if (equipmentEvent != null)
                {
                    OnEquipmentEvent?.Invoke(equipmentEvent);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Error parsing equipment event: {ex.Message}");
            }
        }

        private void HandleEquipmentStateMessage(string data)
        {
            try
            {
        ...
        private void HandleItemUnequippedMessage(string data)
        {
            try
            {
                ...
                if (message != null)
                {
                    OnItemUnequipped?.Invoke(message.PlayerId, message.Slot);
                }
            }
            catch (Exception ex)
            {
        ...
        private void HandleItemCreatedMessage(string data)
        {
            try
            {
                var equipment = JsonUtility.FromJson<Equipment>(data);
                if (equipment != null)
                {
                    OnItemCreated?.Invoke(equipment);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Error parsing item created message: {ex.Message}");
            }
        }
        ...
        private void HandleItemDestroyedMessage(string data)
        {
            try
            {
                ...
                if (message != null)
                {
                    OnItemDestroyed?.Invoke(message.ItemId);
                }
            }
            catch (Exception ex)
            {
        ...
        private void HandleItemEvolvedMessage(string data)
        {
            try
            {
                var equipment = JsonUtility.FromJson<Equipment>(data);
                if (equipment != null)
                {
                    OnItemEvolved?.Invoke(equipment);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Error parsing item evolved message: {ex.Message}");
            }
        }

        private void HandleItemRepairedMessage(string data)
        {
            try
            {
                var equipment = JsonUtility.FromJson<Equipment>(data);
                if (equipment != null)
                {
                    OnItemRepaired?.Invoke(equipment);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Error parsing item repaired message: {ex.Message}");
            }
        }

        private void HandleItemIdentifiedMessage(string data)
        {
            try
            {
                var equipment = JsonUtility.FromJson<Equipment>(data);
                if (equipment != null)
                {
                    OnItemIdentified?.Invoke(equipment);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Error parsing item identified message: {ex.Message}");
            }
        }
