...
namespace VDM.Runtime.Magic.Services
{
    ...
    public class MagicWebSocketHandler : BaseWebSocketHandler
    {
        ...
        protected override void RegisterMessageHandlers()
        {
            ...
            // Magic system events
            RegisterHandler("magic_event", HandleMagicEvent);
            RegisterHandler("magic_state_updated", HandleMagicStateUpdated);
            RegisterHandler("spell_cast", HandleSpellCast);
            RegisterHandler("spell_effect_added", HandleSpellEffectAdded);
            RegisterHandler("spell_effect_removed", HandleSpellEffectRemoved);
            RegisterHandler("spellcaster_updated", HandleSpellcasterUpdated);
            RegisterHandler("mana_changed", HandleManaChanged);
            RegisterHandler("concentration_changed", HandleConcentrationChanged);

            // Room management
            RegisterHandler("magic_session_joined", HandleMagicSessionJoined);
            RegisterHandler("magic_session_left", HandleMagicSessionLeft);
            RegisterHandler("magic_session_ended", HandleMagicSessionEnded);
        }
        ...
        private void HandleMagicEvent(object data)
        {
            try
            {
                var magicEvent = JsonUtility.FromJson<MagicEvent>(data.ToString());
                if (magicEvent != null)
                {
        ...
        private void HandleMagicStateUpdated(object data)
        {
            try
            {
                var magicState = JsonUtility.FromJson<MagicSystemState>(data.ToString());
                if (magicState != null)
                {
        ...
        private void HandleSpellCast(object data)
        {
            try
            {
                var castingResult = JsonUtility.FromJson<SpellCastingResult>(data.ToString());
        ...
        private void HandleSpellEffectAdded(object data)
        {
            try
            {
                var spellEffect = JsonUtility.FromJson<ActiveSpellEffect>(data.ToString());
                if (spellEffect != null)
                {
        ...
        private void HandleSpellEffectRemoved(object data)
        {
            try
            {
                var effectId = data.ToString();
                if (!string.IsNullOrEmpty(effectId))
                {
                    // Remove from cached state
                    _cachedMagicState.GlobalEffects.RemoveAll(e => e.EffectId == effectId);
                    OnSpellEffectRemoved?.Invoke(effectId);
        ...
        private void HandleSpellcasterUpdated(object data)
        {
            try
            {
                var spellcaster = JsonUtility.FromJson<Spellcaster>(data.ToString());
                if (spellcaster != null)
                {
        ...
        private void HandleManaChanged(object data)
        {
            try
            {
                var manaData = JsonUtility.FromJson<ManaChangeData>(data.ToString());
                if (manaData != null)
                {
        ...
        private void HandleConcentrationChanged(object data)
        {
            try
            {
                var concentrationData = JsonUtility.FromJson<ConcentrationChangeData>(data.ToString());
        ...
        private void HandleMagicSessionJoined(object data)
        {
            try
            {
                var sessionData = JsonUtility.FromJson<MagicSessionData>(data.ToString());
                if (sessionData != null)
                {
        ...
        private void HandleMagicSessionLeft(object data)
        {
            try
            {
        ...
        private void HandleMagicSessionEnded(object data)
        {
            try
            {
