...
namespace VisualDM.Entities
{
    public class NPCBase : MonoBehaviour
    {
        ...
        // --- NPC Memory System Integration ---
        private VisualDM.Systems.MemoryManager _memoryManager;
        public VisualDM.Systems.MemoryManager MemoryManager => _memoryManager ??= VisualDM.Systems.NPCMemorySystem.Instance?.GetOrCreateMemoryManager(NPCName);
        ...
        // Motif integration
        public List<VDM.NPC.MotifSystem.Motif> GetActiveMotifs()
        {
            if (MotifManager.Instance == null) return new List<VDM.NPC.MotifSystem.Motif>();
            return MotifManager.Instance.GetMotifsAffectingLocation(transform.position);
        }

        // Example: Query motif effects on relationship, inventory, builder
        public void ApplyMotifEffects()
        {
            var motifs = GetActiveMotifs();
            // Relationship: adjust trust or mood based on motifs
            foreach (var motif in motifs)
            {
                if (motif.Tags.Contains("relationship_boost"))
                {
                    // Example: boost trust
                    foreach (var key in _trustRelationships.Keys)
                        _trustRelationships[key].TrustValue += motif.Intensity;
                }
                if (motif.Tags.Contains("inventory_bonus"))
                {
                    // Example: add motif-specific item (pseudo-code)
                    // Inventory.Add($"MotifItem_{motif.Name}");
                }
                if (motif.Tags.Contains("builder_override"))
                {
                    // Example: modify builder logic (pseudo-code)
                    // CharacterBuilder.ApplyMotifOverride(motif);
                }
            }
        }

        // Debug: print motif effects
        public void DebugPrintMotifEffects()
        {
            var motifs = GetActiveMotifs();
            Debug.Log($"[NPCBase] {NPCName} affected by motifs: {string.Join(", ", motifs.ConvertAll(m => m.Name))}");
        }

        public void AddMemory(string content, float importance, List<string> associatedEntities, float decayRate, List<string> tags, MemoryType type, string source = null, float emotionalImpact = 0f, float reliability = 1f, float falseMemoryProbability = 0f)
        {
            VisualDM.Systems.NPCMemorySystem.Instance?.AddMemory(NPCName, content, importance, associatedEntities, decayRate, tags, type, source, emotionalImpact, reliability, falseMemoryProbability);
        }

        public Memory RecallMemory(Func<Memory, bool> predicate, float contextBoost = 0f)
        {
            return VisualDM.Systems.NPCMemorySystem.Instance?.RecallMemory(NPCName, predicate, contextBoost);
        }

        public void ReinforceMemory(string memoryId, float amount = -1f)
        {
            VisualDM.Systems.NPCMemorySystem.Instance?.ReinforceMemory(NPCName, memoryId, amount);
        }

        public void DecayMemories(float deltaTime)
        {
            MemoryManager?.DecayMemories(deltaTime);
        }
    }
}