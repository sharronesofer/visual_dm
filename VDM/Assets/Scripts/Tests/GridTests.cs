...
namespace VisualDM.Tests
{
    ...
    {
        [Test]
        public void TestCubicToAxialAndBack()
        {
            var hex = new HexCoordinate(2, -3, 1);
            var (q, r) = hex.ToAxial();
            var hex2 = HexCoordinate.FromAxial(q, r);
        ...
        {
            var a = HexCoordinate.FromAxial(0, 0);
            var b = HexCoordinate.FromAxial(3, -2);
            Assert.AreEqual(3, a.Distance(b));
        }

        [Test]
        public void TestNeighbor()
        {
            var hex = HexCoordinate.FromAxial(0, 0);
            var east = hex.Neighbor(0);
            Assert.AreEqual(HexCoordinate.FromAxial(1, 0), east);
        }

        [Test]
        public void TestPOIToRegionAndBack()
        {
            var poi = HexCoordinate.FromAxial(22, 11); // Should be in region (2, 1)
            var region = HexGridUtils.POIToRegion(poi);
            Assert.AreEqual(HexCoordinate.FromAxial(2, 1), region);
            var origin = HexGridUtils.RegionToPOIOrigin(region);
            Assert.AreEqual(HexCoordinate.FromAxial(22, 11), origin);
        }

        [Test]
        public void TestPOIsInRegion_Count()
        {
            var region = HexCoordinate.FromAxial(0, 0);
            int count = 0;
            foreach (var poi in HexGridUtils.POIsInRegion(region))
                count++;
            Assert.AreEqual(121, count); // 121 POI hexes in a region (radius 11)
        }
    }
#endif
}