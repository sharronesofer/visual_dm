...
const getTileKey = (position: Position): str => `${position.x},${position.y}`
...
const mapReducer = (state: MapState, action: ActionType): MapState => {
  switch (action.type) {
    case 'MOVE_PLAYER': {
...
const calculateVisibleTiles = (position: Position, radius: float): string[] => {
  const visibleTiles: List[string] = []
