...
class VideoThumbnailGenerator extends BaseThumbnailGenerator {
  ...
  private getVideoMetadata(videoPath: str): Promise<{ duration: float; format: str }> {
  ...
  private extractFrame(videoPath: str, outputPath: str, timestamp: float): Promise<void> {
    return new Promise((resolve, reject) => {
      ffmpeg(videoPath)
        .screenshots({
  ...
  private async processImage(imagePath: str, options: ThumbnailOptions): Promise<Buffer> {
