...
abstract class BaseService<T extends BaseEntity> implements IBaseService<T> {
  ...
  protected abstract readonly entityName: str
  protected abstract tableName: str
   ...
   */
  async getAll(params?: { filter?: Partial<T>; page?: float; pageSize?: float }): Promise<T[]> {
    try {
   ...
   */
  async getById(id: str | number): Promise<T | null> {
    try {
   ...
   */
  async update(id: str | number, data: Partial<T>): Promise<T> {
    try {
   ...
   */
  async delete(id: str | number): Promise<void> {
    try {
   ...
   */
  async findByIds(ids: List[string]): Promise<T[]> {
    try {
   ...
   */
  async exists(id: str): Promise<boolean> {
   ...
   */
  protected getNextVersion(currentVersion: float): float {
  ...
  async findAll(options?: { limit?: float; offset?: float }, transaction?: Transaction): Promise<T[]> {
    try {
  ...
  async delete(id: str | number, transaction?: Transaction): Promise<void> {
    try {
