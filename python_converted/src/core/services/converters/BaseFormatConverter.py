...
class ResourceTracker:
    tempFiles: Set[str>
    streams: Union[Set[Readable, Writable>]
    timers: Set[NodeJS.Timeout>
abstract class BaseFormatConverter extends TypedEventEmitter<FormatConverterEvents> implements FormatConverter {
  ...
  protected memoryThreshold: float
  constructor() {
    super()
    ...
    this.memoryThreshold = os.totalmem() * 0.8 
  }
  abstract canConvertFrom(mimeType: str): bool
  ...
  protected async readFile(path: str): Promise<Buffer> {
  ...
  protected updateStats(success: bool, input: Buffer | string, output: Buffer | null, time: float): void {
  ...
  protected updateMemoryStats(currentUsage: float): void {
