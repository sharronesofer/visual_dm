...
class RetryConfig:
    ...
    maxDelay?: float
    backoffFactor?: float
    retryableErrors?: Union[List[str, RegExp>]
class RequiredRetryConfig:
    maxAttempts: float
    initialDelay: float
    maxDelay: float
    backoffFactor: float
...
class RetryHandler {
   ...
   */
  private isRetryable(error: Error): bool {
   ...
   */
  private calculateDelay(attempt: float): float {
   ...
   * Promise-based delay
   */
  private sleep(ms: float): Promise<void> {
