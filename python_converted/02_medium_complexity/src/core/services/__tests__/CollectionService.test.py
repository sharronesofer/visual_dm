...
class TestRepository implements Repository<Collection> {
  private items: List[Collection] = []
  private nextId = 1
  async findById(id: str | number): Promise<Collection | null> {
  ...
  async update(id: str | number, entity: Partial<Collection>): Promise<Collection | null> {
