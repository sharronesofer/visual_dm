...
class TestRepository implements Repository<MediaAsset> {
  private items: List[MediaAsset] = []
  private nextId = 1
  async findById(id: str | number): Promise<MediaAsset | null> {
  ...
  async update(id: str | number, entity: Partial<MediaAsset>): Promise<MediaAsset | null> {
