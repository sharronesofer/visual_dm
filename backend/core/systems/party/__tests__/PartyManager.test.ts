...
import { PartyRole, PartyStatus, ResourceSharingPolicy } from '../types';
...
describe('PartyManager', () => {
  ...
  describe('Party Membership', () => {
    test('should add a member to a party', () => {
      ...
      expect(updatedParty!.status).toBe(PartyStatus.ACTIVE); // Should change to active now
    });
    ...
    test('should not add a member if party is full', () => {
      ...
      // Add one member (party size is 2, so this is now full - leader + 1 member)
      ...
      const updatedParty = partyManager.getParty(party.id);
      expect(updatedParty!.members.length).toBe(2); // Still only 2 members (leader + 1)
    });
    ...
    test('should remove a member from a party', () => {
      ...
      expect(updatedParty!.status).toBe(PartyStatus.FORMING); // Back to forming with just the leader
    });
    ...
    test('should not allow removing the leader', () => {
      ...
      const updatedParty = partyManager.getParty(party.id);
      expect(updatedParty!.members.length).toBe(1); // Still has the leader
    });
  });
  ...
  describe('Leadership', () => {
    ...
    test('should not transfer leadership if current leader is incorrect', () => {
      ...
      // Try to transfer with incorrect current leader
      const result = partyManager.transferLeadership(party.id, fakeLeaderId, memberId);
      expect(result).toBe(false);

      const updatedParty = partyManager.getParty(party.id);
      expect(updatedParty!.leaderId).toBe(leaderId); // Still the original leader
    });
  });
  ...
  describe('Party Actions and Quests', () => {
    ...
    test('should complete a quest for a party', () => {
      ...
      expect(updatedParty!.status).toBe(PartyStatus.ACTIVE); // Back to active when no active quests
    });
  });
  ...
  describe('Party Queries', () => {
    test('should get parties by status', () => {
      // Create parties with different statuses
      ...
      // Add a member to party1 to make it active
      const memberId = crypto.randomUUID() as UUID;
    ...
    test('should get parties by member', () => {
      ...
      // Add leaderId as member to party2
  ...
  describe('Party Invitations', () => {
    test('should send and accept invitations', () => {
      ...
      // Check that the invitee is now a member
      const updatedParty = partyManager.getParty(party.id);
    ...
    test('should not allow non-members to send invitations', () => {
      ...
      // Try to send invitation as non-member
      const invitation = partyManager.sendInvitation(party.id, nonMemberId, inviteeId);
      expect(invitation).toBeNull();
    });
  });

  describe('Party Disbanding Scenarios', () => {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    test('should disband party when all members leave', () => {
      const leaderId = crypto.randomUUID() as UUID;
      const memberId1 = crypto.randomUUID() as UUID;
      const memberId2 = crypto.randomUUID() as UUID;
      const party = partyManager.createParty('Disband Test', leaderId);
      partyManager.addMember(party.id, memberId1);
      partyManager.addMember(party.id, memberId2);

      // Remove all members except leader
      partyManager.removeMember(party.id, memberId1);
      partyManager.removeMember(party.id, memberId2);
      // Remove leader last (should not be allowed directly)
      const result = partyManager.removeMember(party.id, leaderId);
      expect(result).toBe(false);
      // Disband via leader command
      const disbandResult = partyManager.disbandParty(party.id, leaderId);
      expect(disbandResult).toBe(true);

      const updatedParty = partyManager.getParty(party.id);
      expect(updatedParty!.status).toBe(PartyStatus.DISBANDED);
      // Ensure party is removed from all entity mappings
      [leaderId, memberId1, memberId2].forEach(id => {
        const parties = partyManager.getPartiesByMember(id);
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    test('should emit PARTY_DISBANDED event on disband', () => {
      const leaderId = crypto.randomUUID() as UUID;
      const party = partyManager.createParty('Event Test', leaderId);
      let eventEmitted = false;
      (partyManager as any).eventEmitter.on('party:disbanded', (disbandedParty: any) => {
        if (disbandedParty.id === party.id) eventEmitted = true;
      });
      partyManager.disbandParty(party.id, leaderId);
      expect(eventEmitted).toBe(true);
    });

    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    test('should handle edge case: leader disconnect (simulate by not disbanding)', () => {
      const leaderId = crypto.randomUUID() as UUID;
      const memberId = crypto.randomUUID() as UUID;
      const party = partyManager.createParty('Edge Case', leaderId);
      partyManager.addMember(party.id, memberId);
      // Simulate leader disconnect: do not call disband, party should persist
      // Remove member
      partyManager.removeMember(party.id, memberId);
      // Party should still exist with leader only
      const updatedParty = partyManager.getParty(party.id);
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    test('should not disband party on quest completion', () => {
      const leaderId = crypto.randomUUID() as UUID;
      const questId = crypto.randomUUID() as UUID;
      const party = partyManager.createParty('Quest Test', leaderId);
      partyManager.addQuestToParty(party.id, questId, leaderId);
      partyManager.completeQuestForParty(party.id, questId);
      const updatedParty = partyManager.getParty(party.id);
      expect(updatedParty!.status).not.toBe(PartyStatus.DISBANDED);
    });
  });
}); 