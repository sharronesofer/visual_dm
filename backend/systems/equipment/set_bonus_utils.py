...
from .models import Equipment, EquipmentSet
...
def apply_set_bonuses(character_stats: Dict[str, Any], set_bonuses: Dict[str, Any]) -> Dict[str, Any]:
    """
    Apply set bonuses to character stats.
    
    Args:
        character_stats: Dictionary containing character stats
        set_bonuses: Dictionary with active set bonuses
        
    Returns:
        Updated character stats with set bonuses applied
    """
    # Make a copy of the stats to avoid modifying the original
    updated_stats = character_stats.copy()
    ...
    for set_id, set_info in set_bonuses.get('active_sets', {}).items():
        for pieces_required, bonuses in set_info.get('active_bonuses', {}).items():
            ...
            for stat, value in bonuses.get('stats', {}).items():
                if stat in updated_stats:
                    updated_stats[stat] += value
                else:
                    updated_stats[stat] = value
