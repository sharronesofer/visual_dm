...
Validation utilities for Arc system data structures and business rules.
"""
...
def validate_arc_data(arc_data: Dict[str, Any]) -> Dict[str, List[str]]:
    """
    Validate arc data structure and business rules.
    ...
    if "total_steps" in arc_data:
        ...
        elif total_steps > 50:  # Reasonable limit
            if "total_steps" not in errors:
                errors["total_steps"] = []
            errors["total_steps"].append("Total steps should not exceed 50")
    ...
    if "current_step" in arc_data:
        ...
        # Cross-validation with total_steps
        if "total_steps" in arc_data and isinstance(arc_data["total_steps"], int):
            if current_step > arc_data["total_steps"]:
                if "current_step" not in errors:
                    errors["current_step"] = []
                errors["current_step"].append("Current step cannot exceed total steps")
...
def validate_step_data(step_data: Dict[str, Any]) -> Dict[str, List[str]]:
    """
    Validate arc step data structure and business rules.
    ...
    # Required fields
    required_fields = ["title", "description", "narrative_text", "step_type", "step_index"]
    for field in required_fields:
    ...
    if "completion_criteria" in step_data:
        criteria = step_data["completion_criteria"]
        if not isinstance(criteria, dict):
            if "completion_criteria" not in errors:
                errors["completion_criteria"] = []
            errors["completion_criteria"].append("Completion criteria must be a dictionary")
...
def validate_progression_data(progression_data: Dict[str, Any]) -> Dict[str, List[str]]:
    """
    Validate arc progression data structure and business rules.
...
def validate_narrative_text(text: str) -> List[str]:
    ...
    if not isinstance(text, str):
        errors.append("Narrative text must be a string")
        return errors
    
    if len(text.strip()) < 10:
        errors.append("Narrative text must be at least 10 characters long")
    
    if len(text) > 5000:
        errors.append("Narrative text should not exceed 5000 characters")
    ...
    if len(words) > 5:
        ...
        for word, count in word_counts.items():
            if count / total_words > 0.1:  # More than 10% repetition
                errors.append(f"Excessive repetition of word '{word}' ({count}/{total_words})")
    ...
    for pattern in placeholder_patterns:
        if re.search(pattern, text, re.IGNORECASE):
            errors.append(f"Contains placeholder text matching pattern: {pattern}")
...
def validate_arc_business_rules(arc_data: Dict[str, Any]) -> List[str]:
    """
    Validate business rules for arc creation and updates.
    ...
    if isinstance(arc_type, ArcType):
        # Global arcs should have high priority
        if arc_type == ArcType.GLOBAL:
            ...
            if priority == ArcPriority.LOW:
                violations.append("Global arcs should not have low priority")
        
        # Character arcs should have character_id
        if arc_type == ArcType.CHARACTER:
            if not arc_data.get("character_id"):
                violations.append("Character arcs must have a character_id")
        
        # Regional arcs should have region_id
        if arc_type == ArcType.REGIONAL:
            if not arc_data.get("region_id"):
                violations.append("Regional arcs must have a region_id")
        
        # Faction arcs should have faction_ids
        if arc_type == ArcType.FACTION:
    ...
    if isinstance(status, ArcStatus):
        completion_percentage = arc_data.get("completion_percentage", 0)
        
        # Completed arcs should have 100% completion
        if status == ArcStatus.COMPLETED and completion_percentage < 1.0:
            violations.append("Completed arcs must have 100% completion")
        
        # Active arcs should have some progress but not be complete
        if status == ArcStatus.ACTIVE and completion_percentage >= 1.0:
