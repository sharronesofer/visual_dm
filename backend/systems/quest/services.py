...
class QuestBusinessService:
    ...
    def delete_quest(self, quest_id: UUID) -> bool:
        ...
        if entity.status == QuestStatus.ACTIVE and entity.player_id:
            raise QuestValidationError("Cannot delete active quest assigned to player")
    ...
    def assign_quest_to_player(self, quest_id: UUID, player_id: str) -> QuestData:
        """Assign quest to player with business rules"""
        # Business rule: Quest must exist and be available
        entity = self.quest_repository.get_quest_by_id(quest_id)
        if not entity:
            raise QuestNotFoundError(f"Quest {quest_id} not found")
        
        if entity.status != QuestStatus.PENDING:
            raise QuestValidationError(f"Quest must be pending to assign, current status: {entity.status.value}")
        
        if entity.player_id:
            raise QuestValidationError(f"Quest is already assigned to player {entity.player_id}")
        
        # Business rule: Check player quest limit
        ...
        if len(player_quests) >= self.config['max_active_quests_per_player']:
            raise QuestValidationError(f"Player has reached maximum active quest limit ({self.config['max_active_quests_per_player']})")
        
        # Assign quest
        entity.player_id = player_id
        entity.status = QuestStatus.ACTIVE
        entity.updated_at = datetime.utcnow()
        
        return self.quest_repository.update_quest(entity)

    def complete_quest_step(self, quest_id: UUID, step_id: int, player_id: str) -> QuestData:
        """Complete a quest step with business validation"""
        # Business rule: Quest must exist and be active for player
        entity = self.quest_repository.get_quest_by_id(quest_id)
        if not entity:
            raise QuestNotFoundError(f"Quest {quest_id} not found")
        ...
        if entity.status != QuestStatus.ACTIVE:
            raise QuestValidationError(f"Quest must be active to complete steps, current status: {entity.status.value}")
        
        # Business rule: Complete the step
        step_completed = entity.complete_step(step_id)
        if not step_completed:
            raise QuestValidationError(f"Step {step_id} not found in quest")
        
        # Business rule: Check if quest is now complete
    ...
    def abandon_quest(self, quest_id: UUID, player_id: str, reason: str = "player_choice") -> QuestData:
        """Abandon quest with business rules"""
        # Business rule: Quest must exist and be assigned to player
        entity = self.quest_repository.get_quest_by_id(quest_id)
        if not entity:
            raise QuestNotFoundError(f"Quest {quest_id} not found")
        ...
        if entity.status not in [QuestStatus.ACTIVE, QuestStatus.PENDING]:
            raise QuestValidationError(f"Cannot abandon quest with status: {entity.status.value}")
        
        # Business rule: Record abandonment
        entity.status = QuestStatus.ABANDONED
        entity.properties = entity.properties or {}
        entity.properties['abandonment_reason'] = reason
        entity.properties['abandoned_at'] = datetime.utcnow().isoformat()
        entity.updated_at = datetime.utcnow()
    ...
    def get_available_quests_for_player(self, player_id: str, location_id: Optional[str] = None) -> List[QuestData]:
        ...
        # Business rule: Get pending quests at location or generally available
        if location_id:
            location_quests = self.quest_repository.get_location_quests(location_id, "pending")
            general_quests = self.quest_repository.list_quests(status="pending", size=100)[0]
            available_quests = location_quests + [q for q in general_quests if not q.location_id]
        else:
            available_quests = self.quest_repository.list_quests(status="pending", size=100)[0]
        
        # Business rule: Filter by unassigned quests
        return [quest for quest in available_quests if not quest.player_id]

    def generate_quest_for_npc(self, npc_data: Dict[str, Any], context: Dict[str, Any]) -> Optional[QuestData]:
        """Generate quest for NPC using generation service"""
    ...
    def calculate_quest_difficulty_score(self, quest: QuestData) -> float:
        """Calculate numeric difficulty score for quest"""
        base_scores = {
            QuestDifficulty.EASY: 1.0,
            QuestDifficulty.MEDIUM: 3.0,
            QuestDifficulty.HARD: 6.0,
            QuestDifficulty.EPIC: 10.0
        }
        
        score = base_scores.get(quest.difficulty, 3.0)
        
        # Adjust for number of steps
        if quest.steps:
            step_multiplier = 1.0 + (len(quest.steps) - 1) * 0.2
            score *= step_multiplier
    ...
    def _is_valid_status_transition(self, current_status: QuestStatus, new_status: QuestStatus) -> bool:
        """Validate quest status transitions"""
        valid_transitions = {
