...
class QuestBaseModel(SharedBaseModel):
    """Base model for quest system with common fields"""
...
class QuestModel(QuestBaseModel):
    """Primary model for quest system"""
    
    name: str = Field(..., description="Name of the quest")
    description: Optional[str] = Field(None, description="Description of the quest")
    status: str = Field(default="active", description="Status of the quest")
    properties: Optional[Dict[str, Any]] = Field(default_factory=dict)


class QuestEntity(Base):
    """SQLAlchemy entity for quest system"""
    
    __tablename__ = f"quest_entities"
    ...
    def __repr__(self):
        return f"<QuestEntity(id={self.id}, name={self.name})>"
