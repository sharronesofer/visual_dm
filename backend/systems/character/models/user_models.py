...
from backend.systems.core_shared.models import CoreBaseModel, Base # Using Base.metadata for tables

# Association table: user_roles
# Assuming User.id and Role.id will be UUIDs from CoreBaseModel
...
# Assuming Role.id and Permission.id will be UUIDs from CoreBaseModel
...
class User(CoreBaseModel):
    ...
    # id, created_at, updated_at are inherited from CoreBaseModel (UUID, Timestamps)

    email = Column(String(255), nullable=False, unique=True)
    password_hash = Column(String(255), nullable=False)
    is_active = Column(Boolean, default=True, nullable=False)
    
    # Fields from old model that are not standard in CoreBaseModel
