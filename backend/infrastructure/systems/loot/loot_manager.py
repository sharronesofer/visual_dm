...
class LootManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.warning(f"Event dispatcher not available: {e}")
    ...
    def initialize(self, config: Dict[str, Any] = None) -> bool:
        ...
        try:
            if config:
                ...
                self.monster_abilities = config.get("monster_abilities", config.get("monster_feats", []))  # Backward compatibility with legacy "feats" terminology
                self.shop_inventories = config.get("shop_inventories", {})
    ...
    def generate_loot(self, location: Dict[str, Any], level: int, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            loot = generate_location_specific_loot(
                ...
                monster_abilities=self.monster_abilities
            )
            ...
            if self.event_dispatcher:
                try:
                    from backend.infrastructure.systems.loot.events.loot_events import LootGeneratedEvent
                    ...
                    self.logger.warning(f"Failed to publish loot generated event: {e}")
    ...
    def identify_item(self, item: Dict[str, Any], character_id: int = None, skill_level: int = 0, character_level: int = 1, method: str = "skill_check", **kwargs) -> Tuple[Dict[str, Any], bool]:
        """
        Item identification system using the new tiered approach (Option B).
        ...
        Args:
            ...
            method: Identification method to use ('skill_check', 'shop_payment', 'auto_level', 'magic')
            **kwargs: Additional parameters based on method (payment_amount, shop_tier, region_id, etc.)
        ...
        try:
            ...
            id_system = TieredIdentificationSystem()
            
            # Map string method to enum
            method_map = {
                "skill_check": IdentificationMethod.SKILL_CHECK,
                "shop_payment": IdentificationMethod.SHOP_PAYMENT,
                "auto_level": IdentificationMethod.AUTO_LEVEL,
                "magic": IdentificationMethod.MAGIC,
                "special_npc": IdentificationMethod.SPECIAL_NPC,
                "quest_completion": IdentificationMethod.QUEST_COMPLETION
            }
            
            identification_method = method_map.get(method, IdentificationMethod.SKILL_CHECK)
            ...
            updated_item, result, message = id_system.identify_item_comprehensive(
                ...
                shop_tier=kwargs.get("shop_tier", 1),
                region_id=kwargs.get("region_id"),
                special_components=kwargs.get("special_components", [])
            )
            ...
            self.logger.info(f"Identification attempt - Method: {method}, Result: {result.value}, Message: {message}")
            
            # Determine success for backward compatibility
    ...
    def enhance_item(self, item: Dict[str, Any], enhancement_type: str, character_id: int = None) -> Tuple[Dict[str, Any], bool]:
        ...
        try:
            ...
            if success:
                ...
                if self.event_dispatcher:
                    try:
                        from backend.infrastructure.systems.loot.events.loot_events import ItemEnhancementEvent, EnhancementResult
                        ...
                        self.logger.warning(f"Failed to publish item enhancement event: {e}")
    ...
    def get_shop_inventory(self, shop_type: str, location: Dict[str, Any], shop_tier: int = 1) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Generate inventory
            from backend.systems.loot.utils.loot_shop import generate_shop_inventory
            ...
            if self.event_dispatcher:
                try:
                    from backend.infrastructure.systems.loot.events.loot_events import ShopInventoryEvent
                    ...
                    self.logger.warning(f"Failed to publish shop inventory event: {e}")
    ...
    def get_loot_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "monster_abilities_count": len(self.monster_abilities)
        }
