"""
World State Database Service - Technical Infrastructure

This module provides database access services for the world state system.
Contains SQLAlchemy models, database operations, and technical infrastructure.
"""
...
from backend.infrastructure.systems.world_state.models import (
    World_StateModel,
    World_StateEntity,
...
class World_StateDatabaseService(BaseService[World_StateEntity]):
    """Database service class for world_state technical operations"""
    ...
    async def get_world_state_by_id(self, world_state_id: UUID) -> Optional[World_StateResponse]:
        ...
        try:
            ...
            logger.error(f"Error getting world_state {world_state_id}: {str(e)}")
            raise
    ...
    async def update_world_state(
        ...
        try:
            ...
            if not entity:
                raise World_StateNotFoundError(f"World_State {world_state_id} not found")
            ...
            logger.error(f"Error updating world_state {world_state_id}: {str(e)}")
            self.db.rollback()
            raise
    ...
    async def delete_world_state(self, world_state_id: UUID) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting world_state {world_state_id}: {str(e)}")
            self.db.rollback()
            raise
...
def create_world_state_database_service(db_session: Session) -> World_StateDatabaseService:
    """Create world_state database service instance"""
    return World_StateDatabaseService(db_session) 