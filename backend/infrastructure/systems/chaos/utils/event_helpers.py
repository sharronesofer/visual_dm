...
class EventHelpers:
    ...
    def calculate_event_cooldown(
        ...
        """
        Calculate cooldown period for an event type.
        ...
        type_modifiers = {
            ChaosEventType.POLITICAL_UPHEAVAL: 1.5,
            ChaosEventType.ECONOMIC_COLLAPSE: 2.0,
            ChaosEventType.WAR_OUTBREAK: 3.0,
    ...
    def generate_event_title(
        ...
        """Generate a title for an event based on type, severity, and context"""
        base_titles = {
        ...
        base_title = base_titles.get(event_type, "Unknown Event")
        
        # Add severity modifier
        if severity == EventSeverity.CATASTROPHIC:
            title = f"Catastrophic {base_title}"
        elif severity == EventSeverity.CRITICAL:
            title = f"Critical {base_title}"
        elif severity == EventSeverity.MAJOR:
            title = f"Major {base_title}"
        else:
            title = base_title
        
        # Add context-specific modifiers
        if 'government_type' in context:
            if event_type == ChaosEventType.POLITICAL_UPHEAVAL:
                title = f"{title} in {context['government_type'].title()}"
        
        return title
    
    @staticmethod
    def determine_affected_regions(
        event_type: ChaosEventType,
        severity: EventSeverity,
        pressure_data: PressureData
    ) -> List[str]:
        """Determine which regions are affected by an event"""
        affected_regions = []
        
        if not hasattr(pressure_data, 'regional_pressures') or not pressure_data.regional_pressures:
            return []
        
        # Determine pressure threshold based on severity
        thresholds = {
            EventSeverity.MINOR: 0.6,
            EventSeverity.MODERATE: 0.5,
            EventSeverity.MAJOR: 0.4,
            EventSeverity.CRITICAL: 0.3,
            EventSeverity.CATASTROPHIC: 0.2
        }
        
        threshold = thresholds.get(severity, 0.5)
        
        # Find regions with pressure above threshold
        for region_id, pressure_info in pressure_data.regional_pressures.items():
            pressure_score = getattr(pressure_info, 'pressure_score', 0.0)
            if pressure_score >= threshold:
                affected_regions.append(str(region_id))
        
        # Ensure at least one region for non-global events
        if not affected_regions and pressure_data.regional_pressures:
            # Pick the region with highest pressure
            highest_pressure_region = max(
                pressure_data.regional_pressures.items(),
                key=lambda x: getattr(x[1], 'pressure_score', 0.0)
            )
            affected_regions.append(str(highest_pressure_region[0]))
    ...
    def calculate_event_severity(
        ...
        """Calculate event severity based on chaos score and pressure sources"""
        # Calculate average pressure from sources
        if pressure_sources:
            avg_pressure = sum(pressure_sources.values()) / len(pressure_sources)
        else:
            avg_pressure = 0.0
        
        # Combine chaos score and pressure
        combined_score = (chaos_score + avg_pressure) / 2.0
