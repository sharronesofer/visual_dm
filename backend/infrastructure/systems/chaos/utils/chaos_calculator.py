...
class ChaosCalculator:
    ...
    def __init__(self, config: Union[Dict[str, Any], object]):
        ...
        Args:
            config: Either a dict with configuration values or a config object with attributes
        """
        if isinstance(config, dict):
            self.config = config
        else:
            # Convert config object to dict for uniform access
            self.config = {
                'chaos_threshold': getattr(config, 'chaos_threshold', 0.7),
            }
