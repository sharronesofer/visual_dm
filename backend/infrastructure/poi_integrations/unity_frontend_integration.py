"""
Unity Frontend Integration Service for POI System

Handles communication and data synchronization between the backend POI system
and Unity frontend, managing real-time updates and visual representation.
"""
...
class UnityFrontendIntegration:
    ...
    def _calculate_global_economic_health(self) -> float:
        ...
        try:
            # Get all active POIs
            active_pois = self.db_session.query(PoiEntity).filter(
                PoiEntity.state.in_(['active', 'growing', 'stable'])
            ).all()
            ...
            for poi in active_pois:
                # Calculate individual POI economic health
                resource_count = len(poi.resources or {})
                population_factor = min(1.0, (poi.population or 0) / 1000)
                trade_factor = 0.5  # Would integrate with trade routes when available
                
                # POI type economic modifiers
                type_modifiers = {
                    POIType.CITY: 1.2,
                    POIType.TOWN: 1.0,
                    POIType.VILLAGE: 0.8,
                    ...
                    POIType.MINE: 0.9,
                    POIType.TEMPLE: 0.7,
                    POIType.FORTRESS: 0.6
                }
                
                type_modifier = type_modifiers.get(poi.poi_type, 1.0)
                ...
                state_modifier = state_modifiers.get(poi.state, 1.0)
                
                poi_health = (
                    (resource_count / 10.0) * 0.4 +
                    population_factor * 0.3 +
                    trade_factor * 0.3
                ) * type_modifier * state_modifier
                
                total_health += min(1.0, poi_health)
            
            return min(1.0, total_health / len(active_pois))
            
        except Exception as e:
            logger.error(f"Error calculating global economic health: {e}")
            return 0.5  # Fallback value
    
    def _calculate_global_political_stability(self) -> float:
        """Calculate global political stability metric"""
        try:
            # Get all POIs with faction data
            pois_with_factions = self.db_session.query(PoiEntity).filter(
                PoiEntity.controlling_faction.isnot(None)
            ).all()
            
            if not pois_with_factions:
                return 0.5  # Neutral stability when no faction control
            
            total_stability = 0.0
            for poi in pois_with_factions:
                # Calculate individual POI political stability
                
                # Base stability by state
                state_stability = {
                    'growing': 0.8,
                    'stable': 0.9,
                    'active': 0.7,
                ...
                base_stability = state_stability.get(poi.state, 0.5)
                
                # Population stability factor (larger populations can be harder to control)
                population = poi.population or 0
                if population < 100:
                    pop_factor = 1.0
                elif population < 500:
                ...
                type_stability = {
                    POIType.FORTRESS: 0.9,  # Military control
                    POIType.CITY: 0.6,      # Complex politics
                    POIType.TOWN: 0.7,      
                    POIType.VILLAGE: 0.8,   # Simpler governance
                    POIType.TEMPLE: 0.8,    # Religious authority
                    POIType.MARKET: 0.6,    # Economic tensions
                    POIType.MINE: 0.7,
                    POIType.OUTPOST: 0.9    # Military discipline
                }
                
                type_factor = type_stability.get(poi.poi_type, 0.7)
                
                # Faction influence stability (would be enhanced with actual faction relationships)
                faction_influences = poi.faction_influences or {}
                if faction_influences:
                    # More balanced influence = more stability
                    influence_values = list(faction_influences.values())
                    if len(influence_values) > 1:
                        # Calculate variance - lower variance = more balanced = more stable
                        mean_influence = sum(influence_values) / len(influence_values)
                        variance = sum((x - mean_influence) ** 2 for x in influence_values) / len(influence_values)
                        influence_factor = max(0.3, 1.0 - (variance / 100.0))  # Normalize variance impact
                    else:
                        # Single faction control - moderate stability
                        influence_factor = 0.7
                else:
                    # No faction influence data - assume moderate stability
                    influence_factor = 0.6
                
                poi_stability = base_stability * pop_factor * type_factor * influence_factor
                total_stability += min(1.0, poi_stability)
            
            global_stability = total_stability / len(pois_with_factions)
            
            # Apply global modifiers (could be enhanced with world events)
            return min(1.0, global_stability)
            
        except Exception as e:
            logger.error(f"Error calculating global political stability: {e}")
            return 0.5  # Fallback value
