#!/usr/bin/env python3
"""
Comprehensive script to fix all broken imports in the backend after refactoring.
This script fixes imports in both test files and system files.
"""
...
def fix_import_line(line, filename):
    ...
    # Pattern 3: Fix specific known broken imports
    # from backend.infrastructure.rules import calculate_dr -> from backend.infrastructure.shared.rules.rules_utils import calculate_dr
    if 'from backend.infrastructure.rules import calculate_dr' in line:
        return line.replace(
            'from backend.infrastructure.rules import calculate_dr',
            'from backend.infrastructure.shared.rules.rules_utils import calculate_dr'
        )
    
    # Pattern 4: Fix direct module imports that may have moved structure
    ...
    # Pattern 5: Fix infrastructure direct module imports
    ...
    # Pattern 6: Fix specific module imports that may have moved
...
def main():
    ...
    # Find all Python files in tests and systems
    file_patterns = [
        'tests/**/*.py',
        'systems/**/*.py',
        'infrastructure/**/*.py',
    ...
    for pattern in file_patterns:
        files = glob.glob(str(backend_root / pattern), recursive=True)
    ...
    print("\n✅ Comprehensive import fixes completed!")
