...
function generateReport(
  ...
  performanceViolations: List[string],
...
function calculatePerformanceScore(metrics: PerformanceMetrics): float {
  const weights = {
    renderTime: 0.3,
    fps: 0.3,
  ...
  const scores = {
    renderTime: Math.max(0, 100 - (metrics.renderTime / 2)), 
    fps: Math.min(100, (metrics.fps / 60) * 100), 
    interactionLatency: Math.max(0, 100 - metrics.interactionLatency), 
    loadTime: Math.max(0, 100 - (metrics.loadTime / 30)), 
    memoryUsage: Math.max(0, 100 - (metrics.memoryUsage / (50 * 1024 * 1024)) * 100), 
  }
  return Math.round(
...
function generateAccessibilityRecommendations(
  ...
  const recommendations: List[string] = []
...
function generateMarkdownReport(report: TestReport): str {
  return `
