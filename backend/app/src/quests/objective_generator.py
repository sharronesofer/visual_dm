...
class ObjectiveGenerationContext:
    ...
    questHistory: List[str]
    difficulty: float
    [key: str]: Any
const recentObjectives: Set<string> = new Set()
function isObjectiveUnique(objective: QuestObjective): bool {
...
function generateKillObjectives(context: ObjectiveGenerationContext): QuestObjective[] {
...
function generateCollectObjectives(context: ObjectiveGenerationContext): QuestObjective[] {
  const { availableTargets, difficulty } = context
  if (!availableTargets.length) return []
  const target = availableTargets[Math.floor(Math.random() * availableTargets.length)]
...
function generateExploreObjectives(context: ObjectiveGenerationContext): QuestObjective[] {
...
function generateDiplomaticObjectives(context: ObjectiveGenerationContext): QuestObjective[] {
  const { availableTargets, difficulty } = context
  if (!availableTargets.length) return []
  const target = availableTargets[Math.floor(Math.random() * availableTargets.length)]
...
function generateObjectiveForType(type: QuestType, context: ObjectiveGenerationContext): QuestObjective[] {
  switch (type) {
    case 'KILL':
 ...
 */
function validateObjective(obj: Any): bool {
