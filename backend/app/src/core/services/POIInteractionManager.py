...
FeedbackType = Union['dialogue', 'quest_accept', 'quest_decline', 'info_acknowledge']
class DialogueOptions:
    text: str
    action: () => None
class QuestInteractionParams:
    title: str
    description: str
...
class POIInteractionManager {
      ...
      options: [
        {
          ...
          action: () => {
            ...
            NPCEventLoggingService.getInstance().logInteraction(
              'system', 
              interaction,
              { quest: Dict[str, Any], interactionType: 'quest_accept' }
            )
          },
        },
        {
          ...
          action: () => {
            ...
            NPCEventLoggingService.getInstance().logInteraction(
              'system', 
              interaction,
              { quest: Dict[str, Any], interactionType: 'quest_decline' }
            )
          },
        },
      ],
  ...
  public generateInteractionsForPOI(poi: POI): POIInteraction[] {
    ...
    const interactions: List[POIInteraction] = []
    if (poi.chunks) {
   ...
   */
  public startInteraction(poiId: str): void {
    if (!poiId) {
   ...
   * @throws Error if POI is not found
   */
  public updatePOI(poiId: str, updates: Partial<POI>): void {
    if (!poiId) {
      throw new Error('POI ID is required')
    }
    const store = usePoiStore.getState()
   ...
   * @throws Error if POI is not found
   */
  public removePOI(poiId: str): void {
    if (!poiId) {
      throw new Error('POI ID is required')
    }
    const store = usePoiStore.getState()
