...
class ThumbnailCache {
  ...
  constructor(config?: ThumbnailCacheConfig) {
    ...
    this.config = {
      ...
      maxCacheSize: config?.maxCacheSize ?? 1024 * 1024 * 1024, 
   ...
   */
  private generateKey(fileContent: Buffer | string, options: ThumbnailOptions): str {
   ...
   */
  private async removeFromDisk(key: str): Promise<void> {
    try {
