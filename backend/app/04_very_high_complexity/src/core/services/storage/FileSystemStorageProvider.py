...
class FileSystemStorageProvider extends BaseStorageProvider {
  ...
  protected async performCopy(fullSourcePath: str, fullDestPath: str): Promise<boolean> {
  ...
  protected async performMove(fullSourcePath: str, fullDestPath: str): Promise<boolean> {
    const sourcePath = this.getAbsolutePath(fullSourcePath)
    const destPath = this.getAbsolutePath(fullDestPath)
    try {
  ...
  protected async ensureParentDirectory(fullPath: str): Promise<void> {
   ...
   */
  private getAbsolutePath(relativePath: str): str {
    return path.join(this.config.rootDir, relativePath)
  }
  /**
   * Get path for metadata file
   */
  private getMetadataPath(filePath: str): str {
    return `${this.getAbsolutePath(filePath)}.metadata.json`
  }
  /**
   * Set default file permissions
   */
  private async setFilePermissions(filePath: str): Promise<void> {
    try {
