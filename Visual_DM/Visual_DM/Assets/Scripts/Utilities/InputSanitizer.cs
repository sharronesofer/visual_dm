...
{
    /// <summary>
    /// InputSanitizer: Centralized input sanitization service for all user-generated content.
    /// 
    /// Rules & Patterns:
    ...
    /// - Always sanitize user input both client-side and server-side. Use InputSanitizer for all user-facing fields.
    /// - For new input fields, select the most appropriate InputSanitizer method or add a new one if needed.
    /// - Use the Validator property on UI.Components.InputField to enforce validation at runtime.
    /// - Log suspicious or rejected input using LogSuspiciousInput for audit and incident response.
    ...
    public static class InputSanitizer
    {
        ...
        public static void LogSuspiciousInput(string userId, string inputType, string payload)
        {
            string message = $"Suspicious input detected. User: {userId}, Type: {inputType}, Payload: {payload}";
            VisualDM.Utilities.ErrorHandlingService.Instance.LogWarning(message, "InputSanitizer.LogSuspiciousInput");
        }
