import { IGenerationPipeline, IPipelineStep, IRandomGenerator } from './IWorldGenerator';

/**
 * Implementation of a generation pipeline that processes input through a series of steps
 */
export class GenerationPipeline<TInput, TOutput> implements IGenerationPipeline<TInput, TOutput> {
    ...
    public execute(input: TInput, random: IRandomGenerator): TOutput {
        if (this.steps.length === 0) {
            // If there are no steps, we can't transform TInput to TOutput
            throw new Error('Cannot execute an empty pipeline');
        }

        // Process the input through each step in the pipeline
