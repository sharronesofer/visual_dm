import { IRandomGenerator, ISeedConfig } from './IWorldGenerator';
 ...
 */
export class DeterministicRNG implements IRandomGenerator {
    ...
    private initState(seed: number): number {
        // Ensure the seed is non-zero (required for xorshift)
    ...
    public createChild(name: string): IRandomGenerator {
        // Create a deterministic but different seed based on the name and parent seed
        let childSeed = this.seedConfig.seed;
        for (let i = 0; i < name.length; i++) {
            childSeed = (childSeed * 31 + name.charCodeAt(i)) | 0;
        }

        // Create a new seed config that references the parent
