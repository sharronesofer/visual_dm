 ...
 */
export interface HandcraftedRegionGeneratorOptions extends RegionGeneratorOptions {
 ...
 */
export class HandcraftedRegionGenerator extends BaseRegionGenerator {
    ...
    protected generateRegion(options: RegionGeneratorOptions, random: IRandomGenerator): Region {
        ...
        // Check if a specific template ID is provided in options
    ...
    private generateCellsFromTemplate(
        ...
        // First, fill the grid with default cells (plains)
        ...
        // Then, apply the template's cell definitions
        for (const templateCell of template.cells) {
            if (templateCell.x >= width || templateCell.y >= height) {
                continue; // Skip cells outside the target dimensions
            }

            // Find the corresponding cell in our cells array
            const cellIndex = templateCell.y * width + templateCell.x;
            if (cellIndex >= 0 && cellIndex < cells.length) {
                // Update the cell with template data
                cells[cellIndex] = {
                    ...cells[cellIndex],
                    ...templateCell
                };
