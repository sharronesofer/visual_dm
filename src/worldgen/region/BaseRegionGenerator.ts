import { GeneratorType, IGenerationResult, ISeedConfig, IRandomGenerator } from '../core/IWorldGenerator';
import { DeterministicRNG } from '../core/DeterministicRNG';
import { IRegionGenerator, Region, RegionGeneratorOptions } from './RegionGeneratorInterfaces';

/**
 * Abstract base class for region generators that implements common functionality
 */
export abstract class BaseRegionGenerator implements IRegionGenerator {
    ...
    public generate(options: RegionGeneratorOptions): IGenerationResult<Region> {
        // Create the seed configuration if not provided
    ...
    protected ensureSeedConfig(options: RegionGeneratorOptions): ISeedConfig {
        ...
        // Generate a seed based on the current time if not provided
