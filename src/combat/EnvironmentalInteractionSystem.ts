...
import { HazardType } from '../worldgen/environment/types';
import { CombatParticipant } from './CombatParticipant';

// Add new interfaces for physics interactions
...
interface PhysicsObject extends InteractiveObject {
...
interface InteractiveObject {
  id: string;
  type: 'destructible' | 'lever' | 'trap' | 'resource' | 'physics';
...
export class EnvironmentalInteractionSystem {
  ...
  interactWithObject(objectId: string, participant: CombatParticipant): boolean {
  ...
  private handleDestructibleInteraction(object: InteractiveObject, participant: CombatParticipant): boolean {
    if (object.health && object.health > 0) {
      object.health -= participant.calculateDamage();
      if (object.health <= 0) {
        object.state = 'destroyed';
        
        // Handle physics object destruction if applicable
  ...
  // Handle lever activation/deactivation
  private handleLeverInteraction(object: InteractiveObject): boolean {
  ...
  private handleTrapInteraction(object: InteractiveObject, participant: CombatParticipant): boolean {
  ...
  private handleResourceInteraction(object: InteractiveObject, participant: CombatParticipant): boolean {
  ...
  // Apply effects from object destruction
  ...
  // Apply effects from lever activation
  ...
  // Apply trap effects to a participant
  ...
  // Apply resource collection effects
  ...
  // Apply an effect to an area
  private applyAreaEffect(center: { q: number; r: number }, effect: { type: string; magnitude: number; radius?: number }): void {
  ...
  // Apply an effect to a specific cell
  private applyCellEffect(cell: TacticalHexCell, effect: { type: string; magnitude: number }): void {
    switch (effect.type) {
      case 'fire':
  ...
  // Add methods for physics interactions
  ...
  applyForce(objectId: string, force: ForceVector): boolean {
    const object = this.physicsObjects.get(objectId);
    if (!object || !object.physics.isMovable) return false;

    // Update velocity based on force and mass (F = ma)
    const acceleration = {
    ...
    object.velocity = {
      ...
      direction: {
        q: (object.velocity.magnitude * object.velocity.direction.q + acceleration.q) / object.velocity.magnitude,
        r: (object.velocity.magnitude * object.velocity.direction.r + acceleration.r) / object.velocity.magnitude
      }
    };
  ...
  private updatePhysicsObjects(): void {
    for (const object of this.physicsObjects.values()) {
      ...
      // Update angular velocity if rotatable
  ...
  private checkCollisions(object: PhysicsObject, newPosition: { q: number; r: number }): PhysicsObject[] {
    const collisions: PhysicsObject[] = [];
    const radius = 1; // Collision check radius
  ...
  private handleCollisions(object: PhysicsObject, collisions: PhysicsObject[]): void {
    for (const other of collisions) {
      // Calculate collision response using restitution
      ...
      // Update velocities based on conservation of momentum and restitution
      if (other.physics.isMovable) {
        ...
        object.velocity = {
          ...
          direction: {
            q: impulse.q / Math.sqrt(Math.pow(impulse.q, 2) + Math.pow(impulse.r, 2)),
            r: impulse.r / Math.sqrt(Math.pow(impulse.q, 2) + Math.pow(impulse.r, 2))
          }
        };
        ...
        if (other.velocity) {
          ...
          other.velocity = {
            ...
            direction: {
              q: otherImpulse.q / Math.sqrt(Math.pow(otherImpulse.q, 2) + Math.pow(otherImpulse.r, 2)),
              r: otherImpulse.r / Math.sqrt(Math.pow(otherImpulse.q, 2) + Math.pow(otherImpulse.r, 2))
            }
          };
        }
      } else {
        // Reflect velocity off immovable object
        object.velocity = {
          magnitude: object.velocity.magnitude * restitution,
          direction: {
            q: -object.velocity.direction.q,
            r: -object.velocity.direction.r
          }
        };
      }
    }
  ...
  // Add physics-based combat methods
  applyPhysicsAttack(source: CombatParticipant, target: PhysicsObject, force: number): boolean {
    const direction = {
    ...
    // Apply force with damage scaling
  ...
  // Add method for rotational forces
  applyTorque(objectId: string, torque: number): boolean {
    const object = this.physicsObjects.get(objectId);
    if (!object || !object.physics.isRotatable) return false;

    // Update angular velocity based on torque and mass
    const angularAcceleration = torque / (object.physics.mass * 0.5); // Simple moment of inertia approximation
    object.angularVelocity += angularAcceleration;

    return true;
  }

  // Add environmental effects on physics objects
        ...
        switch (effect.type) {
          case 'wind':
            ...
            const windForce: ForceVector = {
              magnitude: effect.magnitude * 2, // Scale wind effect to force
              direction: { q: 1, r: 0 } // Assuming wind blows in positive q direction
            };
            this.applyForce(object.id, windForce);
            break;
            
          case 'burning':
            // Reduce mass over time for burning objects
            if (object.physics.mass > 0.1) { // Prevent mass from going too low
              object.physics.mass *= (1 - effect.magnitude * 0.1);
            }
            break;
            
          case 'flooded':
            // Increase friction in water
  ...
  // Modify processTurn to include environmental physics
  processTurn(): void {
    // Apply environmental effects to physics objects
  ...
  // Process effects from active hazards
  ...
  // Process weather effects across the battlefield
  private processWeatherEffects(effect: string, intensity: number): void {
  ...
  // Get all interactive objects within range of a position
  getInteractiveObjectsInRange(position: { q: number; r: number }, range: number): InteractiveObject[] {
  ...
  // Get current environmental effects at a position
  getEnvironmentalEffectsAtPosition(position: { q: number; r: number }): Array<{ type: string; magnitude: number }> {
    const effects: Array<{ type: string; magnitude: number }> = [];
  ...
  // Get the magnitude of a terrain effect
  private getTerrainEffectMagnitude(effect: string): number {
    switch (effect) {
  ...
  // Add method to handle physics object destruction
