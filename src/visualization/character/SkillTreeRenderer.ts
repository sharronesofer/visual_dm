import { Character, CharacterSkill, SkillTreeNode, CharacterUIEvent } from './types';
...
export class SkillTreeRenderer {
  ...
  private readonly nodeRadius: number = 30;
  private readonly nodeSpacing: number = 100;
  ...
  private findNodeAtPosition(x: number, y: number): SkillTreeNode | null {
  ...
  private layoutSkillTree(skills: CharacterSkill[]): SkillTreeNode[] {
    // This is a simple grid layout
    // In a real implementation, you might want to use a more sophisticated
    // layout algorithm like force-directed graph layout
    ...
    skills.forEach((skill, index) => {
      ...
      nodes.push({
        skill,
        position: {
          x: startX + col * this.nodeSpacing,
          y: startY + row * this.nodeSpacing
        },
