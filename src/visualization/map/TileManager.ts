import { Point, Size, Tile, TerrainType, MapData } from './types';
...
export class TileManager {
  ...
  public loadTile(x: number, y: number): Tile | null {
  ...
  public unloadTile(x: number, y: number): void {
    const key = this.getTileKey(x, y);
  ...
  public updateTileVisibility(x: number, y: number, visible: boolean): void {
    const key = this.getTileKey(x, y);
    const tile = this.tiles.get(key);
    if (tile) {
  ...
  public unloadTilesOutsideViewport(viewportStart: Point, viewportSize: Size, buffer: number = 2): void {
