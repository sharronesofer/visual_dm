...
import { regionToTactical } from '../MapTransition';
...
describe('regionToTactical', () => {
  ...
  it('creates correct tactical grid size and center mapping', () => {
    ...
    // Center of tactical grid should map to (3,3) in region grid
  ...
  it('handles edge case at region grid border', () => {
    ...
    // Center at (0,0), size 3 (should only fill part of tactical grid)
    const tactical = regionToTactical(grid, 0, 0, 3);
    // Only (0,0) in region grid maps to (1,1) in tactical grid
