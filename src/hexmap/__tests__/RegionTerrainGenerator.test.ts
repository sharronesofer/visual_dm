...
import { generateRegionTerrain, assignRegionWeather, setRegionDiscovery, revealRegionFrom, isCellVisible, hexGridToMapData } from '../RegionTerrainGenerator';
...
describe('assignRegionWeather', () => {
  ...
  it('can assign all weather types', () => {
    const grid = new HexGrid(20, 20);
    const allTypes: WeatherType[] = ['clear', 'rain', 'snow', 'fog', 'storm', 'windy'];
...
describe('Fog of War / Discovery Utilities', () => {
  ...
  it('revealRegionFrom marks all cells within radius as discovered', () => {
    ...
    // Center and all neighbors should be discovered
    expect(grid.get(2, 2)?.discovered).toBe(true);
    ...
    // Distant cell should not be discovered
    expect(grid.get(0, 0)?.discovered).toBe(false);
  });
