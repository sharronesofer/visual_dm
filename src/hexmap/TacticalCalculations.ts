...
export function lineOfSight(grid: TacticalHexGrid, fromQ: number, fromR: number, toQ: number, toR: number): boolean {
  // Simple hex LOS using Bresenham-like algorithm
...
export function range(grid: TacticalHexGrid, fromQ: number, fromR: number, toQ: number, toR: number): number {
  // Hex distance
  return (Math.abs(fromQ - toQ) + Math.abs(fromQ + fromR - toQ - toR) + Math.abs(fromR - toR)) / 2;
}

export function tacticalPathfind(grid: TacticalHexGrid, fromQ: number, fromR: number, toQ: number, toR: number, maxAP: number): { path: [number, number][], cost: number } | null {
  // A* pathfinding with movementCost and maxAP
...
export function terrainCombatBonus(cell: TacticalHexCell): number {
...
export function areaOfEffect(grid: TacticalHexGrid, centerQ: number, centerR: number, radius: number): [number, number][] {
