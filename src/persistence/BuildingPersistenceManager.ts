...
import { createPersistence, PersistenceHandler } from '../store/utils/persistence';
import { Logger, LogMetadata } from '../utils/logger';
import { SpatialIndex, SearchBounds } from './spatial/SpatialIndex';

/**
 * Configuration options for the BuildingPersistenceManager
 */
interface BuildingPersistenceConfig {
 ...
 */
export class BuildingPersistenceManager {
  ...
  private async initializeTables(): Promise<void> {
    try {
      ...
      // Create building_interiors table
      await this.database.execute(`
  ...
  private async rollbackTransaction(): Promise<void> {
    if (this.database.rollbackTransaction && this.enableTransactions) {
      try {
        ...
        // Don't throw here as we're already handling another error
      }
    }
  }
   ...
   */
  public async loadBuilding(buildingId: string): Promise<Building | null> {
    try {
      // Try persistence store first
   ...
   */
  public async getBuilding(buildingId: string, includeInterior: boolean = false, includeState: boolean = false): Promise<Building | null> {
    try {
      // Get building data
      const results = await this.database.query(
        'SELECT * FROM buildings WHERE id = ?',
   ...
   */
  public async getAllBuildings(includeInterior: boolean = false, includeState: boolean = false): Promise<Building[]> {
    try {
      // Get all buildings
      const results = await this.database.query('SELECT * FROM buildings');
      const buildings = results.map(result => this.mapDatabaseToBuilding(result));
  ...
  public async deleteBuilding(buildingId: string): Promise<void> {
    try {
      ...
      // Delete from database (cascades to interiors and states)
      await this.database.execute(
   ...
   * Save building state
   */
  public async saveBuildingState(buildingId: string, state: BuildingState): Promise<void> {
    try {
      await this.beginTransaction();
   ...
   * Load building state
   */
  public async loadBuildingState(buildingId: string): Promise<BuildingState | null> {
    try {
      // Try persistence store first
   ...
   */
  public async saveBuildingInterior(buildingId: string, interior: BuildingInterior): Promise<void> {
    try {
      await this.beginTransaction();
   ...
   */
  public async loadBuildingInterior(buildingId: string): Promise<BuildingInterior | null> {
    try {
      // Try persistence store first
   ...
   */
  public async loadCompleteBuildingData(buildingId: string): Promise<Building | null> {
    try {
      ...
      // Load state and interior in parallel
  ...
  public async saveCompleteBuildingData(building: Building): Promise<void> {
    try {
      ...
      // Save state and interior if present
      if (building.state) {
   ...
   */
  public findNearestBuildings(x: number, y: number, k: number = 5): Building[] {
