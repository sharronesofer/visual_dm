 ...
 * Supports validation, querying, and export for visualization.
 */

export type DependencyType = 'prerequisite' | 'exclusive' | 'narrative';

export interface QuestDependencyEdge {
  to: string;
  type: DependencyType;
...
export class QuestDependencyGraph {
  ...
  addDependency(from: string, to: string, type: DependencyType, meta?: Record<string, any>) {
    this.addQuest(from);
    this.addQuest(to);
  ...
  getDependencies(questId: string): QuestDependencyEdge[] {
