...
import { FactionProfile, FactionData } from './types';
import { FactionType as FactionTypeEnum } from '../../types/factions/faction';
...
export class FactionService {
   ...
   */
  constructor(initialData: FactionData[] = []) {
   ...
   */
  public updateRelationship(factionId: FactionType, targetId: FactionType, change: number): number {
   ...
   */
  public updateResource(factionId: FactionType, resource: string, amount: number): void {
    const faction = this.factions.get(factionId);
    if (!faction) throw new Error('Faction not found');
   ...
   */
  public getResource(factionId: FactionType, resource: string): number {
    const faction = this.factions.get(factionId);
    if (!faction) throw new Error('Faction not found');
   ...
   */
  public deserialize(data: FactionData[]): void {
    this.factions.clear();
 ...
 * - updateRelationship(factionId, targetId, change): number
 * - getRelationships(factionId): Record<FactionType, number>
 * - updateResource(factionId, resource, amount): void
 * - getResource(factionId, resource): number
 * - serialize(): FactionData[]
 * - deserialize(data: FactionData[]): void
 *
