...
export class MedievalAnimationController implements AnimationController {
  private state: AnimationState;
  private animationFrame: number | null = null;
  private startTime: number | null = null;
  private handlers: AnimationEventHandlers = {};
  private lastFrameTime: number = 0;
  private readonly targetFPS: number = ANIMATION_DEFAULTS.targetFPS;
  private readonly frameInterval: number = ANIMATION_DEFAULTS.frameInterval;
  private frameCount: number = 0;
  private fpsHistory: number[] = [];
  private performanceWarningIssued: boolean = false;
  ...
  constructor() {
    ...
    this.state = {
      ...
      direction: 'forward',
      layers: {
        foreground: this.getDefaultLayer(),
  ...
  public reverse(): void {
    if (this.state.config) {
      ...
      this.state.config.layers = {
        foreground: {
          ...layers.foreground,
          opacity: Array.isArray(layers.foreground.opacity) 
            ? [layers.foreground.opacity[1], layers.foreground.opacity[0]]
            : layers.foreground.opacity
        },
  ...
  private animate = (): void => {
    ...
    const duration = ANIMATION_TIMING.DURATION.NORMAL; // Default duration
  ...
  private interpolateValue(value: AnimationValue, progress: number): number {
  ...
  private interpolateLayer(config: LayerConfig, progress: number): LayerConfig {
  ...
  private updateLayers(progress: number, config: AnimationConfig): void {
  ...
  private applyEasing(progress: number, easing: string): number {
    // Use a lookup table for common easing values
    const easingLookup: { [key: number]: { [key: string]: number } } = {
      0: { 'easeInOut': 0, 'easeIn': 0, 'easeOut': 0 },
      0.25: { 'easeInOut': 0.125, 'easeIn': 0.0625, 'easeOut': 0.4375 },
      0.5: { 'easeInOut': 0.5, 'easeIn': 0.25, 'easeOut': 0.75 },
      0.75: { 'easeInOut': 0.875, 'easeIn': 0.5625, 'easeOut': 0.9375 },
      1: { 'easeInOut': 1, 'easeIn': 1, 'easeOut': 1 }
    };

    // Check if we can use the lookup table
