...
describe('SpatialLayoutGenerator', () => {
  ...
  describe('generateLayout', () => {
    ...
    test('generates connected paths between POIs', () => {
      ...
      // Check that all POIs have at least one connection
      ...
      for (const path of result.paths) {
        ...
        for (let i = 1; i < path.length; i++) {
          ...
          expect(dx + dy).toBeLessThanOrEqual(1); // Adjacent cells only
        }
      }
    });
    ...
    test('generates layouts with balanced category distribution', () => {
      ...
      // Check that each category has at least one POI
    ...
    test('respects pattern-specific placement rules', () => {
      ...
      for (const placement of socialPlacements) {
        ...
        expect(distance).toBeLessThanOrEqual(10); // Within cluster radius
      }

      // Check linear pattern (EXPLORATION)
      ...
      if (explorationPlacements.length >= 2) {
        ...
        // Project points onto pattern direction and check they're roughly aligned
        ...
        for (let i = 1; i < projections.length; i++) {
          ...
          expect(angleDiff).toBeLessThan(30); // Allow some deviation
        }
      }
    });
  });
