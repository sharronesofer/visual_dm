...
import { BufferManager, BufferEventType } from '../../../services/media-conversion/BufferManager';
import { BaseConverter } from '../../../services/media-conversion/BaseConverter';
import { ConversionEventType, ConversionStage, MediaInput, MediaOutput, BaseConfig } from '../../../services/media-conversion/types';
import { TypedEventEmitter } from '../../../utils/TypedEventEmitter';

// Mock media data for testing
const createMockMediaData = (size: number = 1024): Buffer => {
...
class MockConverter extends BaseConverter {
  ...
  protected async performConversion(input: Buffer): Promise<Buffer> {
    // Simulate conversion by reversing the buffer
...
describe('Media Processing Pipeline Integration', () => {
  ...
  describe('End-to-End Processing', () => {
    it('should process media through the complete pipeline', async () => {
      ...
      // Monitor buffer transformation events
  ...
  describe('Performance and Resource Management', () => {
    it('should handle large files efficiently', async () => {
      const largeData = createMockMediaData(1024 * 1024); // 1MB
      ...
      expect(duration).toBeLessThan(2000); // Should process 1MB in less than 2 seconds
    });
