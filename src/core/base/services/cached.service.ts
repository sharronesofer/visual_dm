...
import { ICacheableService } from '../interfaces/cached.interface';
import { CacheOptions, ServiceResponse, PaginatedResponse, QueryParams } from '../types/common';

/**
 * Abstract cached service class that extends BaseService and implements ICacheableService
 */
export abstract class CachedService<T extends BaseEntity> extends BaseService<T> implements ICacheableService<T> {
  ...
  // Override base methods to add caching
  ...
  async create(data: Omit<T, keyof BaseEntity>): Promise<ServiceResponse<T>> {
    ...
    if (response.success && response.data) {
      this.clearCache(); // Invalidate all cache on create
    }
    return response;
  }
  ...
  async update(id: T['id'], data: Partial<T>): Promise<ServiceResponse<T>> {
    ...
    if (response.success && response.data) {
      this.invalidateCache(id); // Invalidate cache for this entity
    }
    return response;
  }
