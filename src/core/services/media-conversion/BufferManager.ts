 ...
 */
export type MediaBuffer = Buffer | ArrayBuffer | Uint8Array;
...
interface BufferEvents {
  [BufferEventType.TRANSFORM_START]: { inputSize: number };
  [BufferEventType.TRANSFORM_PROGRESS]: { bytesProcessed: number; totalBytes: number };
  [BufferEventType.TRANSFORM_COMPLETE]: { outputSize: number };
 ...
 */
export class BufferManager extends TypedEventEmitter<BufferEvents> {
      ...
      write: (chunk: unknown, encoding: BufferEncoding, callback: (error?: Error) => void) => {
        try {
          ...
          this.emit(BufferEventType.TRANSFORM_PROGRESS, {
            ...
            totalBytes: totalLength // Final size unknown until complete
          });
          callback();
        } catch (error) {
          const bufferError = new BufferError(
   ...
   */
  private *chunkBuffer(buffer: Buffer, chunkSize: number): Generator<Buffer> {
