...
import { BufferManager, MediaBuffer, BufferTransformConfig } from './BufferManager';
...
interface ConverterEvents {
  ...
  [ConversionEventType.COMPLETE]: void;
  [ConversionEventType.WARNING]: string;
}
 ...
 */
export abstract class BaseConverter extends TypedEventEmitter<ConverterEvents> implements IMediaConverter {
  protected supportedInputFormats: string[] = [];
  protected supportedOutputFormats: string[] = [];
   ...
   */
  public validateConfig(config: MediaConverterConfig): boolean {
   ...
   */
  protected emitProgress(stage: ConversionStage, percent: number): void {
  ...
  protected async createInputStream(input: MediaInput, config?: BufferTransformConfig): Promise<Readable> {
    ...
    // Handle buffer types using BufferManager
