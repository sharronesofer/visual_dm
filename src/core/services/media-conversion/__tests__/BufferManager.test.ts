import { BufferManager, BufferEventType, MediaBuffer } from '../BufferManager';
  ...
  describe('Event Handling', () => {
    ...
    it('should emit progress events for readable stream', done => {
      ...
      readable.on('end', () => {
        ...
        expect(completeHandler).not.toHaveBeenCalled(); // Complete is only emitted for streamToBuffer
        done();
      });

      readable.resume(); // Start flowing mode
    });
