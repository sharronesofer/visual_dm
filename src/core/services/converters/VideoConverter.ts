...
export class VideoConverter extends BaseFormatConverter {
  ...
  private readonly defaultOptions: ConversionOptions = {
    targetFormat: 'mp4',
    quality: 80, // Maps to CRF value (0-51, lower is better)
    compressionLevel: 6, // Maps to preset (0-9, higher is slower/better)
    chunkSize: 1024 * 1024, // 1MB chunks
    preserveMetadata: false,
    preserveAspectRatio: true
  };

  private readonly presetMap: { [key: number]: string } = {
    0: 'ultrafast',
    1: 'superfast',
    2: 'veryfast',
    3: 'faster',
  ...
  private async getVideoMetadata(input: string): Promise<ffmpeg.FfprobeData> {
  ...
  public async convertStream(
    ...
    try {
      ...
      // Process the video and save to temporary output file
      await new Promise<void>((resolve, reject) => {
        command
          .on('error', (err) => reject(err))
          .on('end', () => resolve())
