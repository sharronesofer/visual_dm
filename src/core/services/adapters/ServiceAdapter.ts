import { BaseEntity, BaseQueryParams, CacheOptions, ServiceResponse as BaseServiceResponse } from '../base.service';
import { IService, ServiceParams } from '../base/IService';
import { ServiceConfig, ServiceResponse, ValidationError, ServiceError } from '../base/types';
...
> extends BaseServiceAdapter<T, CreateDTO, UpdateDTO> implements IService<T, CreateDTO, UpdateDTO> {
  ...
  public handleValidationError(error: ValidationError[]): ServiceResponse<T> {
  ...
  public async getById(id: string): Promise<ServiceResponse<T>> {
  ...
  public async update(id: string, data: UpdateDTO): Promise<ServiceResponse<T>> {
    const validationErrors = await this.validate(data);
    if (validationErrors.length > 0) {
      return this.handleValidationError(validationErrors);
    }
