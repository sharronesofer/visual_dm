...
import { POIIntegrationSystem } from '../systems/POIIntegrationSystem';
import { POIType, POISubtype } from '../poi/types/POITypes';

// Add mapping for POIType and POISubtype at the top
...
// Helper to create a plain POI object
function createPOIObject(params: {
  id: string;
  name: string;
  type: POIType;
  size: any;
...
class POIGenerationService {
  ...
  private async generateLayout(params: POIGenerationParams): Promise<POILayout> {
    ...
    rooms.push({
      ...
      properties: {
        name: 'Entrance',
        description: `The entrance to this ${params.type}`,
      },
    });
    for (let i = 1; i < numRooms; i++) {
      ...
      connections.push({
        ...
        properties: {
          ...
          reverseDirection: this.getOppositeDirection('door'),
        },
      });
      rooms.push(room);
    }
  ...
  private calculateEntrancePosition(dimensions: { width: number; height: number }): Position {
    return {
      x: Math.floor(dimensions.width / 2),
      y: dimensions.height - 1, // Bottom center
    };
  }
  ...
  private calculateExitPositions(
    ...
    // For simplicity, place exit at top center
    return [
      {
        x: Math.floor(dimensions.width / 2),
        y: 0,
  ...
  private calculateRoomPosition(
    dimensions: { width: number; height: number },
    existingRooms: POIRoom[]
  ): Position {
    // Simple position calculation - in real implementation, this should use
    // proper room placement algorithms to avoid overlaps
    return {
  ...
  private getRoomType(params: POIGenerationParams, index: number): string {
    const roomTypes = {
  ...
  private calculateRoomCount(params: POIGenerationParams): number {
    const baseCount = {
      tiny: 3,
      small: 5,
      medium: 8,
    ...
    for (let i = 0; i < totalRooms; i++) {
      ...
      for (let j = 0; j < numFeatures; j++) {
        features.push({
          ...
          position: {
            x: room.position.x + Math.floor(this.random() * room.width),
            y: room.position.y + Math.floor(this.random() * room.height),
          },
          properties: {
  ...
  private getRandomFeatureType(params: POIGenerationParams): string {
    const featureTypes = {
      ...
      for (let j = 0; j < numEntities; j++) {
        ...
        entities.push({
          ...
          properties: {
            ...
            stats: {
              health: 100,
              level: Math.floor(this.random() * 10) + 1,
            },
            inventory: [],
            interactions: [],
          },
  ...
  private getRandomEntityType(params: POIGenerationParams): string {
    const entityTypes = {
  ...
  public async generatePOI(params: POIGenerationParams): Promise<ReturnType<typeof createPOIObject>> {
    try {
      ...
      // Integrate building and road generation for POI
      ...
      const integrationResult = POIIntegrationSystem.generateForPOI({
        ...
        thematicElements: {
          ...
          dangerLevel: 5,
          lighting: 'bright',
          themes: [params.theme || '', params.type || '']
        }
      });
      ...
      // Attach buildings and roads to layout
  ...
  private distributeToRooms(layout: POILayout, features: POIFeature[], entities: POIEntity[]) {
    let featureIndex = 0;
    let entityIndex = 0;
...
export default POIGenerationService;
