...
import { FileNotFoundError, FileExistsError, StorageError } from '../interfaces';
...
describe('FileSystemStorageProvider', () => {
  ...
  describe('getUrl', () => {
    it('should return file URL', () => {
      ...
      expect(url).toBe(`file://${join(testDir, 'test.txt')}`);
    });
  });
  ...
  describe('move', () => {
    ...
    it('should handle cross-device moves', async () => {
      // Mock EXDEV error to simulate cross-device move
