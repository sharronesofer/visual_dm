...
import { MonsterAttack, MonsterAttackDirection, MonsterAttackOutcome } from '../types/monster';
 ...
 */
export class MonsterSiegeService {
  private static instance: MonsterSiegeService;
  private attacks: MonsterAttack[] = [];
  private directions: MonsterAttackDirection[] = [
    'north',
    'northeast',
    'southeast',
    'south',
   ...
   */
  public triggerAttacks(pois: POI[], options?: { seed?: number; attackChance?: number }): MonsterAttack[] {
    const attacks: MonsterAttack[] = [];
    const rng = options?.seed !== undefined ? this.seededRandom(options.seed) : Math.random;
    const attackChance = options?.attackChance ?? 0.2; // 20% base chance

    for (const poi of pois) {
      if (!this.isEligiblePOI(poi)) continue;
      // Calculate vulnerability (placeholder: random for now)
      const vulnerability = rng();
      if (vulnerability < attackChance) {
        // Select monster types (placeholder: static for now)
   ...
   */
  private isEligiblePOI(poi: POI): boolean {
    // Example: Only active POIs, not already under attack, not destroyed
  ...
  private selectMonsterTypes(poi: POI, rng: () => number): string[] {
    // TODO: Integrate with actual monster selection logic (see app/combat/monster_utils.py)
    // For now, return a single generic monster
    return ['goblin'];
  }
   ...
   */
  public async resolveAttack(attackId: string): Promise<{ outcome: MonsterAttackOutcome; combatLog: string[] }> {
    ...
    // Mock: Retrieve POI and defending NPCs (stubbed values)
   ...
   */
  private trackPoiDamage(poi: POI, damageAmount: number): void {
    const meta = { ...(poi.metadata || {}) };
   ...
   */
  private convertPoiType(poi: POI, newType: string): void {
   ...
   */
  private recordStateChange(poi: POI, cause: string): void {
    const meta = { ...(poi.metadata || {}) };
    if (!meta.stateHistory) meta.stateHistory = [];
    meta.stateHistory.push({ timestamp: new Date(), state: { ...poi }, cause });
    poi.setMetadata(meta);
  }
   ...
   */
  private notifyStateChange(poi: POI, changeType: string): void {
    // In a real system, push to notification service or event bus
   ...
   */
  private calculateDamage(outcome: MonsterAttackOutcome): number {
   ...
   */
  private mockGetPOI(poiId: string): POI {
    // For now, create a new POI with metadata for each call (stateless mock)
   ...
   */
  private mockDefenderStrength(poiId: string): number {
    // For now, use a deterministic value based on POI ID hash for testability
    let hash = 0;
    for (let i = 0; i < poiId.length; i++) {
      hash = ((hash << 5) - hash) + poiId.charCodeAt(i);
      hash |= 0;
    }
    return 8 + (Math.abs(hash) % 8); // Range: 8-15
  }
} 