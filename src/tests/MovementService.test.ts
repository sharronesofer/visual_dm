...
import { PlayerMovementState } from '../types/player';
...
describe('MovementService', () => {
  ...
  describe('getAvailableMoves', () => {
    test('returns all valid adjacent positions', () => {
      ...
      expect(moves.length).toBe(4); // Only cardinal directions when diagonal is disabled
    });
    ...
    test('excludes obstacle positions', () => {
      ...
      const moves = movementService.getAvailableMoves(position, 30);
      expect(moves).not.toContainEqual({ x: 5, y: 5 }); // Obstacle position
    });
  ...
  describe('updatePlayerState', () => {
    ...
    test('maintains state after failed move', () => {
      const targetPosition: Position = { x: 5, y: 5 }; // Obstacle position
      const newState = movementService.updatePlayerState(
        initialState,
        targetPosition
      );
