...
export class DialogueManager {
  ...
  private getEncouragement(style: string): string {
  ...
  public async generateNegotiationResponse(npc: NPCData, offerQuality: number): Promise<string> {
    const baseResponses = {
      excellent: [
  ...
  public async generateDeceptionResponse(npc: NPCData, detected: boolean): Promise<string> {
    const responses = {
      detected: [
  ...
  public async generateCompetitionResponse(
    npc: NPCData,
    response: { accepted: boolean; intensity: string; terms?: any }
  ): Promise<string> {
    const responses = {
      accepted: {
        high: [
          "Challenge accepted! Prepare yourself!",
          "I'll show you what real competition is!",
