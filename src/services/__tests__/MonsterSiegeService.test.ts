import { MonsterSiegeService } from '../MonsterSiegeService';
...
describe('MonsterSiegeService', () => {
  ...
  it('generates attacks for eligible POIs', () => {
    ...
    const attacks = service.triggerAttacks(pois, { seed: 42, attackChance: 1 }); // 100% chance for test
  ...
  it('resolves attack with outcome: defended', async () => {
    ...
    // Set attack strength low to guarantee defense
    attack.strength = 5;
  ...
  it('updates POI state after attack', async () => {
    ...
    // Simulate a decisive defeat
    const [attack] = service.triggerAttacks([poi], { seed: 5, attackChance: 1 });
    attack.strength = 1; // Make sure defenders lose
    await service.resolveAttack(attack.id);
