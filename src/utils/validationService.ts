import { CharacterData, ValidationState, Skill, Equipment, StarterKit } from '../types/character';
import { validateEquipmentSelection, validateEquipmentCompatibility } from './equipmentValidation';
...
// Validation steps for character creation
export const characterCreationSteps: ValidationStep[] = [
  {
    key: 'race',
    required: true,
    validate: data => {
  ...
  },
  {
    ...
    validate: data => {
      ...
      // Additional class-specific skill validation could go here
  ...
  },
  {
    ...
    validate: data => {
      ...
      // Additional class-specific equipment validation could go here
...
// Check if it's safe to proceed to the next step
...
export class ValidationService {
  ...
  private validateSkills(
    ...
    // Calculate expected skill count based on class and background
    ...
    // Validate expertise (if applicable)
  ...
  private getExpectedSkillCount(characterClass: string, background: string): number {
    ...
    switch (characterClass.toLowerCase()) {
      ...
      case 'wizard':
        ...
        break;
      default:
        count += 2; // Default for other classes
    }

    // Background-based skill proficiencies
    switch (background.toLowerCase()) {
      ...
      case 'soldier':
        ...
        break;
      default:
        count += 2; // Default for other backgrounds
    }
  ...
  public validateField(fieldName: string, value: any, rules: ValidationRules): ValidationResult {
    const result: ValidationResult = {
      isValid: true,
      errors: [],
      warnings: [],
