import { IBaseService, ServiceHooks } from '../types/services';
import {
  ApiResponse,
  ValidationResult,
  ServiceConfig,
...
import { createCancellationToken } from '../utils/http';
...
> implements IBaseService<T, CreateDTO, UpdateDTO>
{
  ...
  async create(data: CreateDTO): Promise<ApiResponse<T>> {
    try {
      ...
      // Execute after hook if successful
   ...
   * @param id The ID of the entity to delete
   */
  protected abstract executeDelete(id: string): Promise<ApiResponse<void>>;
