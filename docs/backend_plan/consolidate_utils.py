...
TOP_LEVEL_UTILS = os.path.join(BACKEND_DIR, "utils")
SRC_UTILS = os.path.join(BACKEND_DIR, "src", "utils")
...
def consolidate_utils():
    """Consolidate utils folders."""
    # Check which utils folder is more commonly imported
    top_level_imports = check_imports("utils")
    src_imports = check_imports("src.utils")
    
    print(f"Top-level utils imports: {top_level_imports}")
    print(f"src.utils imports: {src_imports}")
    
    # Determine which folder to keep as the primary one
    if top_level_imports >= src_imports:
        primary_dir = TOP_LEVEL_UTILS
        secondary_dir = SRC_UTILS
        print("Keeping top-level utils as primary")
    else:
        primary_dir = SRC_UTILS
        secondary_dir = TOP_LEVEL_UTILS
        print("Keeping src.utils as primary")
    ...
    # If primary dir is src.utils, create symlink from top-level to src
    if primary_dir == SRC_UTILS and os.path.exists(secondary_dir):
        shutil.rmtree(secondary_dir)
        # Copy src.utils to top-level instead of creating symlink (which can be problematic)
