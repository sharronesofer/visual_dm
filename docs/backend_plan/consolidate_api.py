...
TOP_LEVEL_API = os.path.join(BACKEND_DIR, "api")
SRC_API = os.path.join(BACKEND_DIR, "src", "api")
...
def consolidate_api():
    """Consolidate API folders."""
    # Check which API folder is more commonly imported
    top_level_imports = check_imports("api")
    src_imports = check_imports("src.api")
    
    print(f"Top-level API imports: {top_level_imports}")
    print(f"src.api imports: {src_imports}")
    
    # Determine which folder to keep as the primary one
    if top_level_imports >= src_imports:
        primary_dir = TOP_LEVEL_API
        secondary_dir = SRC_API
        print("Keeping top-level API as primary")
    else:
        primary_dir = SRC_API
        secondary_dir = TOP_LEVEL_API
        print("Keeping src.api as primary")
    ...
    # If primary dir is src.api, create symlink from top-level to src
    if primary_dir == SRC_API and os.path.exists(secondary_dir):
        shutil.rmtree(secondary_dir)
        # Copy src.api to top-level instead of creating symlink
