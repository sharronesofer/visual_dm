...
TOP_LEVEL_CORE = os.path.join(BACKEND_DIR, "core")
SRC_CORE = os.path.join(BACKEND_DIR, "src", "core")
...
def consolidate_core():
    """Consolidate core folders."""
    # Check which core folder is more commonly imported
    top_level_imports = check_imports("core")
    src_imports = check_imports("src.core")
    
    print(f"Top-level core imports: {top_level_imports}")
    print(f"src.core imports: {src_imports}")
    
    # Determine which folder to keep as the primary one
    if top_level_imports >= src_imports:
        primary_dir = TOP_LEVEL_CORE
        secondary_dir = SRC_CORE
        print("Keeping top-level core as primary")
    else:
        primary_dir = SRC_CORE
        secondary_dir = TOP_LEVEL_CORE
        print("Keeping src.core as primary")
    ...
    # If primary dir is src.core, create symlink from top-level to src
    if primary_dir == SRC_CORE and os.path.exists(secondary_dir):
        shutil.rmtree(secondary_dir)
        # Copy src.core to top-level instead of creating symlink
