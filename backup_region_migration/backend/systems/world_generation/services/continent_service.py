...
from backend.infrastructure.systems.region.schemas import RegionCreationSchema

from backend.infrastructure.systems.world_generation.models import (
    ContinentCreationRequestSchema,
    ContinentSchema,
    ContinentBoundarySchema
)
from backend.infrastructure.systems.world_generation.repositories.continent_repository import continent_repository, ContinentRepository
from backend.systems.world_generation.utils.world_generation_utils import (
...
# Mock classes for testing - these should be replaced with actual implementations
class RegionService:
    def create_new_region(self, region_data, continent_id=None):
        return {"id": "mock_region_id", "name": "Mock Region"}

class RegionCreationSchema:
    def __init__(self, coordinates=None, biome=None):
        self.coordinates = coordinates or {"x": 0, "y": 0}
        self.biome = biome or "temperate"
